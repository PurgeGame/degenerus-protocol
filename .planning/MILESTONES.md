...
## v75.0 Ticket/Entry Correctness + Disambiguation (Shipped: 2026-06-30)

Fixed a confirmed ¼ ticket under-delivery in both prize legs (`JackpotModule._jackpotTicketRoll` + `LootboxModule._lootboxTicketCount`→queue via `DecimatorModule`) by routing through canonical `wholeTicketsToEntries(w)=w<<2`. Made the ticket/entry distinction unmistakable across the codebase via a 5-phase rename + ABI + repack + entry-granularity sweep. **Verdict: 0 critical / 0 high / 0 medium; CLEAN.** 3-model adversarial re-audit (Codex/GPT-5 + GLM-5.2 + Claude-lens 5-dimension) all converged clean. The 5 gated contract diffs: **479** value fix (`b2ab3e9f`), **480** internal rename sweep 502/502 symmetric (`bcc47ccc`), **481** event/ABI surface + view selectors (`94322027`), **482** Degenerette dead-mode repack EV-byte-identical (`310bccfc`), **483** FF-salvage entry-granularity 5-site coupled change (`61e40429`). 6 phases (479–484) · 7 plans · 39 commits · 136 files · +6,254/−1,790 lines · 1 day (2026-06-29 → 2026-06-30). Test floors: forge **1005/0/107** · Hardhat **1362/0** · layout `--check` green · 37 Bernoulli EV assertions green. Closure: `MILESTONE_V75_AT_HEAD_87a6dd76` (`795fa7ed` with doc fixes). 4 cosmetic nits (optional cleanup, not blockers). **Tag `v75.0` (local; pushed 2026-06-30).** Archived `milestones/v75.0-{ROADMAP,REQUIREMENTS,MILESTONE-AUDIT}.md`.

## v74.0 As-Built Milestone Audit + C4A Package (Shipped: 2026-06-27)

Full milestone audit of the entire **`v73.0 → HEAD` contract batch** (29 `.sol`, +1873/−1032 — the sDGNRS level-start lootbox + pre-deploy hardening: VRF-death deadman, mid-day-RNG fold, 44h admin gate, permissionless-settlement relaxation, named-error migration, whaleBundle→whalePass + WWXRP renames, the in-scope-dormant gas faucet) that had never been milestone-audited, plus assembly of the complete Code4rena package. **Verdict: 0 open findings.** Posture = the subject is already committed; the milestone *verifies* it. Method = verify → freeze-confirm → harness-green → 8-cluster as-built code audit → manifest re-point → cross-model adversarial re-audit (Codex; Gemini CLI tier-ineligible) → live-agent/soak re-attest → C4A package → terminal. **The sole conditional contract gate (475) fired once:** Codex surfaced a **MEDIUM** — `DegenerusAdmin.vote()`/`canExecute()` kill-on-recovery was *lazy* (only invalidated a VRF-coordinator-swap proposal when poked while currently recovered), so a proposal created in stall-1 could survive an un-poked VRF recovery and execute in a later ≥44h re-stall at the age-decayed (→5%) threshold with stale votes — installing an arbitrary VRF coordinator below the governance bar. Owner-approved logic-only fix `93d17288` (also invalidate when `lastVrfProcessed > createdAt` ⇒ any fulfillment after creation kills the proposal, recovery-proof without a poke; regression test `83a5ce43`). The other 5 Codex surfaces clean / by-design / refuted (the >120d VRF-death prevrandao super-fallback by-design → KNOWN-ISSUES §3a; queue-gate / post-gameover ticket insertion refuted; sDGNRS-box pre-rng-request sizing refuted; affiliate ≤3-FLIP quest rounding immaterial). The 8 as-built clusters (SUBJ/HARN/SOLV/RNG/LIVE/ACCESS/EV/WIRE/GAS) all HOLD via isolated neutral-prompt reviewers + an adversarial-verify pipeline + the skeptic filter (Workflow `wf_00bd2866`); contracts git-verified unmodified after each read-only fan-out. **Harness:** forge 0-fail; Hardhat 1359/0/18 (3 genesis-stall guards documented-skipped); VRFGovernance green. **Manifest:** MAN-01 re-pinned v73→fixed-tree, **28→34 invariants** (incl. the REDEEM-05 sDGNRS-storage-restructure fix); `MAIN-INVARIANTS.md` byte-synced to `invariants.json`. **Soak:** live agent re-pointed at a local-fork of the fixed tree → fresh local soak (80 actions) = **0 final MAN-01 violations / 0 profit-vs-EV alarms** (the prior 1000+-step 0-viol soak carries; the lone governance delta is orthogonal to every soaked invariant). **C4A package:** `scope.txt` (50 sources, 20,070 nSLOC) + `out_of_scope.txt` + `SECURITY.md` + trust-model + `KNOWN-ISSUES.md` + `audit/C4A-CONTEST-README.md` (Main-Invariants verbatim from MAN-01) + `audit/ACCESS-CONTROL-MATRIX.md` + `audit/ETH-FLOW-MAP.md`, all against the fixed tree. **Subject byte-frozen** `contracts/` tree **`f06b1ef6`** @ impl **`93d17288`** (the v73→HEAD batch + the one milestone governance fix, the sole `.sol` delta vs `3986926c`); closure `MILESTONE_V74_AT_HEAD_93d17288ba6719e0a77723d6167c0ba4796b8467` (distinct from the v73 tree `d6615306`). 13 phases (466-478). Deliverables `audit/FINDINGS-v74.0.md` (chmod 444) + `audit/AUDIT-V74-REPORT.html`; archived `milestones/v74.0-{ROADMAP,REQUIREMENTS}.md` (+ the superseded "C4A Readiness" plan 457-465 → `v74.0-superseded-plan-{ROADMAP,REQUIREMENTS}.md`). Baseline = v73.0 closure (`d6615306` @ `64ec993e`). **Tag `v74.0` (local; not pushed — USER pushes).**

## v73.0 Degenerette "Variant-2" Color-Gated Rescore (+ WWXRP Rig Preservation) (Shipped: 2026-06-21)

Ports the color-gated-by-symbol scoring rule (shipped + audited on the foil match in v72, `16225de6`) into the **core Degenerette betting engine** — the main paying slot moves ~1-in-3 (32%) → **~1-in-5 (~19.5%) at ~2× the multipliers, at IDENTICAL EV**. DEC-01 R2 WWXRP rig (score-bearing pool, **+2 color-unlock, never a 9** via the m≥7 cap); DEC-02 Option B (honest ETH/FLIP family split per `(N, hero-is-gold)` → 8 base + 8 S8 + 8 factor tables, every pick **exactly EV-equal**, closing the ~+2.24% hero-placement edge); DEC-03 floor S≥2. **Held byte-fixed:** P(S=9) + jackpot pins, WWXRP RTP curve (70→115→118→120%), activity ROI curve (90→99.9%), S=9 whale-pass bracket. **Verdict: 0 CAT / 0 HIGH / 0 MED / 0 LOW; 0 open findings.** Method: 452 GEN (generator-first, no contract edit; EVEQ-01 drift 2.99→0.00007 centi-x → escalated to Option B; P(S=9)/RTP pre-proof) → 453 IMPL (sole gate, USER-approved `64ec993e`) → 454 TST → 455 REAUDIT → 456 TERMINAL. 454: rewrote the v72-era oracles to v73 + new invariant proofs — 44/44 byte-reproduce, per-(N,heroIsGold) EV≤100 & exactly EV-equal, bonus EV=5.000%, P(S=9) invariant, INV-01..04 byte-unchanged vs pre-v73, **3000-spin REAL-rig distribution parity**; full forge **943/0/108** (fixed the 2 v72-era worst-case-gas oracles: win predicate old→Variant-2 + single-pass search). 455: 3 isolated subagents (Solvency/RNG-integrity/Liveness-no-brick, neutral prompts) **+ cross-model Codex** (1,024-state / 2,532-fired-path rig enumeration, max post-rig S=8, real finding none) — converged. **Subject byte-frozen** `contracts/` tree `d6615306` @ IMPL `64ec993e`; closure `MILESTONE_V73_AT_HEAD_15650b6a05427517981b14ac62ddf18364c0525b`. Deliverables `audit/FINDINGS-v73.0.md` (chmod 444) + `audit/AUDIT-V73-REPORT.html`. Baseline = v72.0 closure (`4407181d` @ `e94f1719`). Carries (pre-existing, NOT v73): 6 stale `test:stat` surface anchors (Jackpot/TraitUtils/EntropyLib/lootbox) + `_deployProtocol` real-clock setUp flake (no `block_timestamp` pin). UNPUSHED (USER pushes).

## v72.0 As-Built Audit — Foil Pack + Degenerette WWXRP-Rig / Variant-2 Rescore (+ Gas) (Shipped: 2026-06-21)

Audit of the entire post-v70 contract surface (18 `.sol`, +2,186/−355: foil pack `f255d56c`, WWXRP reel rig + payout fork `1dd07c4d`, Variant-2 foil-match rescore `16225de6`). **Verdict: 0 CAT / 0 HIGH / 0 MED / 0 LOW; 0 open findings.** Method = 6 isolated top-model VERIFY agents (independently re-deriving all load-bearing math) + **Codex (ChatGPT) cross-model** on the 3 load-bearing claims (gemini CLI dead this round). Five candidates raised + all closed: **F-01** 4-of-4 steer = by-design §V.3 (bounded edge, USER-signed); **F-02** dead `traitFromWordFoil`/`packedTraitsFoil` = REMOVED (USER-approved, bytecode-neutral); **F-03** foil-drain budget = Codex liveness HOLDS (no change); **F-04** foil ETH frozen-pool "uncapped" = WITHDRAWN by USER (pendingFuture structurally ≪ pool; exploratory fix reverted); **CG-1** two-distinct-heroes = Codex HOLDS. Pillars all attested: Solvency (rig EV=100 all tables, none overpay; foil legs capped/mint/pool-neutral), RNG integrity (all consumers frozen-at-commitment; P(S9) byte-identical rigged-vs-honest), Liveness (advanceGame not foil-brickable; storage all-appended, forge-inspect). Gas: hot-path Pick 4 applied; new surface already lean. **Subject byte-frozen** `contracts/` tree `4407181d` @ HEAD `e94f1719`; closure `MILESTONE_V72_AT_HEAD_e94f1719a52441ac4dc90a5a6304f09533fa2c96`. FREEZE = sole gate: gas Pick 4 `19dc6390` + dead-code removal `e94f1719`, both USER-approved. forge 942/0/108; Hardhat compiles clean; deploy 14/14. Deliverable `audit/FINDINGS-v72.0.md`. Baseline = v70.0 closure (`99f2e53f` @ `ffbd7796`). UNPUSHED (USER pushes).
...
**Phases (397-405):** 397 FOUNDATION ✅ (green baseline 888/0/110, subject freeze, v63 priors) · 398 LEVEL-SEMANTICS ✅ (~200-site census; **1 CONFIRMED LOW FIXED** — afking lootbox streak-basis off-by-one `891f7a8f` — + USER-raised redemption day+1 multiplier FIXED) · 399 REWARD-MECHANICS ✅ (RWD-A INFO: BURNIE seed ~7.87M emission, codex-unique catch, USER-confirmed by-design) · 400 SOLVENCY·CARRY·REDEMPTION ✅ (identity/carry-escrow/salvage/stETH-before-ETH CEI convergent + 5 compositions) · 401 PACKING & GAS-IDENTITY ✅ (narrowings ≥ real max; 2+4 leads refuted vs uint128 supply cap) · 402 PERMISSIONLESS-COMPOSITION ✅ (3 INFO by-design) · 403 RNG-FREEZE SPINE ✅ (DOMINANT; activity score frozen-at-deposit anti-gaming knob) · 404 MUTATION (bounded + CI-resumable; v63 spine 0-defects carries) · 405 TERMINAL ✅.

**VERDICT: 0 CATASTROPHE / 0 HIGH / 0 MEDIUM. 1 LOW found + FIXED in-milestone. All else INFO/by-design/refuted.** The cross-model council earned its keep twice (RWD-A seed-emission EV + the 401 narrowing flags); codex usage-capped mid-run then fully backfilled.

**Closure signal:** `MILESTONE_V64_AT_HEAD_891f7a8f8bacb163140ebcd7abbb001fb4d1366d` (subject `402855e1` @ `891f7a8f`). Canonical deliverable: `audit/FINDINGS-v64.0.md` (chmod 444) + `AUDIT-V64-REPORT.html`. **ARCHIVED** → `.planning/milestones/v64.0-{ROADMAP,REQUIREMENTS,MILESTONE-AUDIT}.md` (live REQUIREMENTS.md removed; phase dirs kept in `.planning/phases/397-405`, archive via `/gsd-cleanup`). Milestone-completion audit PASSED (38/38 reqs, 9/9 phases, integration PASS, 0 orphans/contradictions). **Tag `v64.0`; NOT pushed** — USER review + push pending. SEED-001 (century streak-shield) captured forward; Lootbox/Decimator mutation tail CI-resumable.
