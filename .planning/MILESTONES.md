...
**Next-milestone handoff:** 1 routed open gated item (BURNIE-04 — the post-audit USER-hand-reviewed contract fix, batched, never pre-approved; 5 pending USER design decisions to resolve first; the fix-design's regression plan flips from characterizing the gap to asserting the fix, re-attested against `test/REGRESSION-BASELINE-v63.md`). BURNIE-05 USER-WONTFIX (operational runbook). The folded v50/v51/v52 legacy debt is DISCHARGED (`FINDINGS-v50.0.md` + `FINDINGS-v51.0.md` authored). 3 RNG/v63-changed mutation modules CI-deferred/resumable (surface already covered by the dual-net).
