...
- **D-10 (TKT-02):** DROP the afking-ticket century/x00 quantity-bonus parity (`DegenerusGameMintModule.sol:1243`) for simplicity. Manual buyers keep it; afking-ticket buyers do not.
...
| C3 | Ticket-mode primitive parity | "Afking ticket subs will use a minimal `_queueTicketsScaled`-equivalent write (no `purchaseWith`), dropping the century/x00 bonus. Does dropping the century parity or deferring affiliate/quest to the settle create any exploit or resolution mismatch vs the manual ticket leg?" |
...
| C5 | Long-run gas | "Suggest gas optimizations for the per-buy accrue (one warm SSTORE into a dedicated accumulator slot), the ~10-day settle leg, and the `OPEN_BATCH` open walk — without weakening any unmanipulability invariant." |
