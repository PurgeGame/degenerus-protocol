...
### Ticket-Mode Primitive (TKT-02) — KEEP century parity (flipped 2026-06-01)
- **D-10 (USER-chosen; flipped 2026-06-01 in plan-phase after a gas analysis):** The new ticket-mode minimal-write primitive **KEEPS the afking-ticket century/x00 quantity-bonus at PARITY** with manual ticket buys (the `targetLevel % 100 == 0` bonus, `DegenerusGameMintModule.sol:1243-1259`). Rationale: the gas cost is amortized-negligible — the bonus is gated by the century-level check (fires only every 100th level), reuses the EXISTING `centuryBonusLevel`/`centuryBonusUsed` storage (`DegenerusGameStorage.sol:1563`/`:1567` — NO new slot), and reuses the per-buy activity score already computed for the affiliate taper (AFF-02); the only real cost (~7–31k, the `centuryBonusUsed[buyer]` zero→nonzero write) lands on the rare century-level buy. The original "drop for simplicity" was about primitive-simplicity, not gas; with the cost shown negligible, parity is the fairer choice. SPEC locks: the ticket primitive applies the century bonus before queuing (same shape as `MintModule:1242-1259`, reusing the existing storage + the computed score).
...
- `contracts/modules/DegenerusGameMintModule.sol` — the O1 re-credit: `:1222-1232` (`handlePurchase` call + `lootboxFlipCredit += questReward`), `:1366-1367` (the batched `creditFlip`). Also `:1243-1259` the century/x00 bonus (**KEPT at parity for afking**, D-10 flipped 2026-06-01 — reuses existing `centuryBonusLevel`/`centuryBonusUsed` storage + the per-buy score), `:1730` the `handleMint` caller (audited clean).
...
- USER posture throughout: minimize everyday writes/gas wherever it stays unmanipulable; favor simplification (drop dead code; the in-slot re-packed accumulator) over behavior-identity — but KEEP parity where the gas cost is negligible (the century bonus, D-10 flipped after the gas analysis showed it's amortized-free).
