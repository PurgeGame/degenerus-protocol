...
>
> **Status (2026-05-27): RE-PLANNED + ready to execute.** The 9 redesign plans (330-01..09; the 7 pre-redesign plans were cleared) are the live set; plan-checker PASSED. The superseded held-330 working-tree diff was RESET (tree is clean, contracts byte-identical to `0cc5d10f`) and ARCHIVED at **`.planning/held-330-superseded.patch`** (recover via `git apply`). That patch is the **SURVIVOR-code reference**: reproduce the advance re-home/`(mult,rewardable)` tuple, the `degeneretteResolve` rename + ≥3 re-peg, GASOPT-01 `[rk]` hoists, and the `advanceDue()`/`boxesPending()` views FROM it (they're unchanged by the redesign); author the REWORKED router/bounty/order parts FRESH per 329-SPEC §3. Execution HARD-STOPS at 330-09 (BATCH-02 hand-review) before any contract commit.
