...
- **D-26 (SALVAGE-01 design):** split the cash leg `cashWei` ("non-ticket value") into an ETH part + a BURNIE part. Randomize a TARGET BURNIE portion full-range `[0 .. cashWei]` (USER) from a NEW slice of the existing prior-day `seed` (no new VRF); `ethCap` = the existing cash ≤60%-of-`totalBudget` limit (so ETH out ≤ `cashWei` ≤ cap always; BURNIE NOT counted against it). **The BURNIE leg is paid from sDGNRS-OWNED BURNIE — NOT a `creditFlip` mint (USER 2026-06-04, salvage-specific):** the sources are sDGNRS's BURNIE token balance (`burnie.balanceOf(SDGNRS)`, via `transfer`/`transferFrom`) + sDGNRS's claimable coinflip credit (`coinflipAmount(SDGNRS)`/`previewClaimCoinflips`, consumed like the existing sDGNRS-stake-consume at `BurnieCoinflip:908`/`consumeCoinflipsForBurn:366`), valued via `_ethToBurnieValue(part, mintPrice())` @ current eth-equivalent. **Funding fallback:** `actualBurnie = min(targetBurnie, sDGNRS-available)`; pay `actualBurnie` from sDGNRS-owned BURNIE and the REMAINDER (`cashWei − actualBurnie`) as ETH. If sDGNRS has ZERO BURNIE + ZERO claimable coinflip → no BURNIE leg, pay the whole cash leg in ETH ("do it more like now"). Always cap-compliant (`ethOut = cashWei − actualBurnie ≤ cashWei ≤ ethCap`). The ticket leg ("ticket-value") is UNCHANGED. Update BOTH the execute path (`MintModule:929/975`) and the preview/quote (`_quoteFarFutureSwap`) — the preview must reflect the source-availability + fallback so the offer stays truly knowable-in-advance.
...
- **D-29 (SALVAGE solvency framing + no-arb re-proof):** SOLVENCY-positive/neutral + NO new BURNIE emission — only the ETH part (`cashWei − actualBurnie`) is relabeled out of `claimableWinnings[SDGNRS]` (≤ the full `cashWei`, so the ETH liability drops vs today; claimablePool ≤ balance holds); the BURNIE part is a TRANSFER of sDGNRS-OWNED BURNIE (token balance and/or its claimable coinflip stake) to the player — no `creditFlip` mint, no inflation. The source-availability check + ETH fallback guarantee the swap never over-draws sDGNRS's BURNIE and stays cap-compliant. The load-bearing obligation owned at SALVAGE-02 (verified at SALVAGE-03): **re-prove the no-arb ceiling under the new variability** — EXTEND `test_SWAP08_NoArbAtCeiling_SweepAllDistances` so the max reachable offer still sits below the far-ticket acquisition floor; since the split conserves the cash-leg VALUE (just changes form/source) the total-value ceiling is unchanged, but the proof must cover the full split range + the BURNIE-source valuation + the fallback path.

### CANCEL — manual sub-cancel auto-claim (self + affiliate tree); auto-evict pure-forfeit (CANCEL-02 SPEC; CANCEL-01 IMPL@359, CANCEL-03 TST@361) — ADDED mid-358

> Afking-subscription cleanup. VERIFIED vs `1e7a646d` via a dedicated scout. BURNIE-emission only, OFF the ETH path, `rngLock`-gated → SOLVENCY-untouched + RNG-freeze-clean (the clean original-v57 posture, unlike BURNIE/SALVAGE).

- **D-30 (the latent loss bug it fixes):** manual cancel (`GameAfkingModule.sol:345-362`, `subscribe(…, dailyQuantity=0)`) calls `_finalizeAfking` + tombstones (`dailyQuantity=0`) and leaves `pendingBurnie` + `affiliateBase` in the slot — a comment (`:348-351`) promises "claim whenever" — BUT the next `processSubscriberStage` reclaim `:1144` `delete _subOf[player]` wipes both → the accruals are LOST if any advance runs before the player (+ an affiliate caller) claim. The comment is false; the auto-claim closes the race.
- **D-31 (CANCEL-01 manual-cancel auto-claim):** on manual cancel, BEFORE clearing: (a) pay the canceller's `pendingBurnie` to themselves via `coinflip.creditFlip` + the presale-box-credit parity (mirror `claimAfkingBurnie:1560`; zero the field first — CEI); (b) drain `affiliateBase` + settle the affiliate tree **A/U1/U2 = 75/20/5** (mirror `DegenerusAffiliate.claim:629` via `drainAffiliateBase:1605`; A = the canceller's referrer — the base is owed UPLINE, NOT to the canceller; deterministic, no roll); (c) `_finalizeAfking` + clear.
- **D-32 (CANCEL-01 auto-evict = pure FORFEIT — USER):** auto-evict (pass-expiry `:1175` + funding-out `:1226` + tombstone-reclaim `:1144`) stays "just delete the data" — delete the Sub record incl. BOTH accumulators, NO payout to self OR uplines. Make the forfeit EXPLICIT: today the pass/funding evict paths `_removeFromSet` but leave `_subOf` claimable out-of-set (a latent inconsistency); the forfeit intent means they must wipe the accruals (delete `_subOf`) so they're truly unclaimable post-evict. (USER accepted that auto-evict denies innocent uplines their share — the distinction is chose-to-leave vs got-kicked.)
- **D-33 (CANCEL freeze/solvency):** BURNIE-emission only (`creditFlip`/`drainAffiliateBase`); reads no `rngWord`; no ETH/`claimablePool` touch (SOLVENCY-01 untouched); cancel is `rngLock`-gated at the `subscribe` entry (`:300`); CEI (zero before credit). No new freeze surface.
