...
> **⚠ READ SPEC §U FIRST (authoritative, USER sign-off 2026-06-19).** Three findings surfaced during 446 planning (grounded in the live `ffbd7796` code) are RESOLVED in `445-SPEC.md` §U and SUPERSEDE the cited body lines: **§U.1** the resolution producer is `traitFromWord@MintModule:759` (NOT `packedTraitsFromSeed`) → foil resolution uses `traitFromWordFoil(s, multBps)` (single-trait), `packedTraitsFoil` stays buy-side-only; **§U.2** the eligibility window + the `recLevel` param + `buyDay`/`levelStartDay` (below); **§U.3** D-446-01 RESOLVED = separate foil queue + leftover-budget drain (below). These widen the edit surface to `MintModule` + `AdvanceModule` (§U.4) — intrinsic to FOIL-05 + MATCH-02, not a scope choice.

### IMPL-discretion residue (the SPEC's open knobs — these are the ONLY 446 choices)
- **D-446-01 (foil-owed jackpot-trait resolution mechanism) — RESOLVED (§U.3):** a **separate append-only `foilTicketQueue[wk]` + `foilOwedPacked[level][player]`** (NOT the normal `ticketQueue`), drained inside `processTicketBatch` on **leftover budget** after the normal queue (with a `foilCursor` for resume), resolving each via the **shared** `_raritySymbolBatch` (gains a `uint16 multBps` param) whose one producer line `:759` branches `multBps==0 ? traitFromWord(s) : traitFromWordFoil(s, multBps)`; frozen `multBps` from `foilRecord`; `FOIL_SEED_TAG` seed separation; the jackpot readiness gate requires **both** queues drained. Chosen over the per-entry tag (avoids a per-player SLOAD on the hot normal drain) and a duplicated batch body (avoids the EIP-170 hit). The v70-frozen producers stay untouched (RARE-01).
- **D-446-W (eligibility window + `recLevel`) — RESOLVED (§U.2):** `claimFoilMatch` gains an explicit `recLevel` param (4-arg); stamp `buyDay` `uint24` at `foilRecord` bits `[168-191]`; maintain a new `mapping(uint24 => uint24) levelStartDay` written at the `level = lvl` transition (`AdvanceModule:1732`); eligible window = `[buyDay+1, levelStartDay[recLevel+1])`.
...
- `contracts/storage/DegenerusGameStorage.sol` — tail-append (all after `boxPlayers` `:2393`, no slot move/retype): `foilRecord` (`mapping(uint24 => mapping(address => uint256))`, now stamping `buyDay` at payload bits `[168-191]` per §U.2), `foilMatchClaimed` (`mapping(bytes32 => bool)`), **`levelStartDay` (`mapping(uint24 => uint24)`)**, **`foilTicketQueue` (`mapping(uint256 => address[])`, `wk`-keyed)**, **`foilOwedPacked`** (small per-`(level,player)` counter, or folded into `foilRecord` reserved bits), **`foilCursor`**; new constants `FOIL_TO_FUTURE_BPS = 2500` + the keccak-domain tags (`FOIL_SEED_TAG`, `FOIL_MAG_TAG`, `FOIL_CCY_TAG`, bonus-traits + ETH-cap `1000` bps as applicable). `level` = `uint24 public level` (`:236`).
- `contracts/modules/DegenerusGameMintModule.sol` (§U.1/§U.3 spine touch) — `_raritySymbolBatch` (`:720`) gains a `uint16 multBps` param + the producer branch at `:759` (`traitFromWord` ↔ `traitFromWordFoil`); `processTicketBatch` gains the leftover-budget foil-drain over `foilTicketQueue[wk]` (`take`/`maxT`/`baseOv` budget `:627-634`) + `foilCursor` resume + the both-queues-drained readiness gate. `_queueTicketsScaled` analog at `:1153` (def `Storage:641-686`); `baseKey` `:592-595`; seed `:745-747`; `traitBurnTicket` append `:795-814`.
- `contracts/modules/DegenerusGameAdvanceModule.sol` (§U.2 spine touch) — one `levelStartDay[lvl] = day` SSTORE at the `level = lvl` transition (`:1732`); the readiness gate / jackpot-draw guard reads `foilCursor` alongside `ticketCursor`.
- `contracts/DegenerusTraitUtils.sol` (root — NOT `contracts/libraries/`; the SPEC's `libraries/` prefix is a grounding typo, corrected by the planner) — NEW siblings `traitFromWordFoil(uint64 s, uint16 multBps)` (the single-trait RESOLUTION producer — §U.1) + `packedTraitsFoil(uint256 rand, uint16 multBps)` (the BUY-side 4-quadrant packer for the `foilRecord` match sigs; calls `traitFromWordFoil` per lane), cloned structurally from `packedTraitsDegenerette`/`_degTrait` (`:201-223`); ONLY the color stage changes (the `/15360` cutoff ladder). The frozen producers `weightedColorBucket`/`traitFromWord`/`packedTraitsFromSeed` (`:115/:143/:169`) are NOT edited/retyped/moved (RARE-01).
