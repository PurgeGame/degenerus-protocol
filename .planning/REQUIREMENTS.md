...
- [ ] **DGN-01**: New `packedTraitsDegenerette(uint256 seed) internal pure returns (uint32)` helper added to `contracts/DegenerusTraitUtils.sol` with per-quadrant producer `[16,16,16,16,16,16,16,8]/120` (commons 13.33%, gold 6.67%) and uniform symbol distribution `1/8`. `[QQ][CCC][SSS]` byte layout preserved. Library `internal pure` is inlined into the consumer at compile time — zero new public-ABI selector. Existing `packedTraitsFromSeed` byte-identical (Mint + Jackpot paths UNCHANGED).
...
- [ ] **DGN-03**: `_countGoldQuadrants(uint32 ticket) private pure returns (uint8)` helper added to `DegenerusGameDegeneretteModule.sol` — counts color==7 across 4 player-pick quadrants by reading bits 5-3 of each packed byte (`((ticket >> (q*8 + 3)) & 7) == 7` for q∈{0,1,2,3}). Returns N ∈ {0..4} for table dispatch.
...
- [x] **REG-04**: Prior-finding spot-check. Walk every prior `audit/FINDINGS-v25..v36.0.md` for findings referencing the v37-touched function set: `DegenerusGameDegeneretteModule` (`_evNormalizationRatio`, `_getBasePayoutBps`, `_applyHeroMultiplier`, `_wwxrpBonusRoiForBucket`), `DegenerusTraitUtils.packedTraitsFromSeed`, `DegenerusGameLootboxModule._resolveLootboxRoll`, presale-flag handling, `setDecimatorAutoRebuy`. Default expectation: ALL rows PASS. SUPERSEDED rows allowed with explicit successor cite (e.g., F-25-NN normalization-related rows replaced by 5-table design).
