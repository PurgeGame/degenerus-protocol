...
### AGT — adversarial agent, local-fork bug-finder (Phase 462)

- [ ] **AGT-01**: The agent boots from the existing local deploy — runs `deploy:local`, loads the exported manifest + per-contract ABIs from `deployments/localhost-abis/`, and connects funded wallets (no hand-rolled typing).
...
- [ ] **AGT-06**: Against a local fork the agent compresses game-time (advance 15-min-days fast), funds instantly, and `evm_snapshot`/`evm_revert`s around each hypothesized exploit; every action is logged as a structured replayable record so a flagged violation is already a runnable, reproducible tx sequence.
- [ ] **AGT-07**: The existing Foundry invariant campaign runs in parallel as a free breadth-adversary (`FOUNDRY_PROFILE=deep`, 1000×256, auto-shrunk failing sequences), alongside the daemon.
