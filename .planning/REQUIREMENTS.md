...
- [ ] **GAS-05**: The per-sub quest+buyer-bonus BURNIE payout is DEFERRED out of the auto-run to a player PULL (USER override 2026-06-01, mid-355): `_settleQuest` keeps advancing the `settleAfkingQuest` streak on the ~10-day cadence and draining `questProgress`/`buyerOwedBurnie`, but accrues the owed BURNIE into a `pendingBurnie` accumulator instead of an inline `creditFlip`; a permissionless claim entrypoint does a final settle of any un-drained progress then pays `pendingBurnie` in ONE `creditFlip` and zeroes it (always credits the sub, never the caller — idempotent, like `claimQuest`). The keeper `mintBurnie` bounty stays an immediate push (quest+buyer-bonus only). Auto-run only — the manual-buy path is unchanged. Off the solvency path (BURNIE-emission-timing only; SOLVENCY-01 untouched). SUPERSEDES the AGG-02 inline settle-day `creditFlip`.
