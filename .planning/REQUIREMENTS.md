...
- [ ] **LBX-EVT-04**: `LootboxTicketRoll` field semantics: `lootboxIndex` is the per-lootbox storage index from `openLootBox(player, index)` / `openBurnieLootBox(player, index)`; `0` for the two non-indexed paths (`resolveLootboxDirect` for decimator claims + `resolveRedemptionLootbox` for sDGNRS redemption) which have no per-index storage slot. `preRollTickets` is the post-distress-bonus, pre-collapse scaled value (i.e. the value snapshotted in LBX-WT-02 as `scaledPre`); divide by `TICKET_SCALE=100` for the floor-tickets, modulo `100` for the round-up Bernoulli weight (e.g. `247` → floor `2`, frac `47`). `roundedUp` is `true` only when the Bernoulli condition `(uint8(seed >> 152) % 100) < uint8(frac)` evaluated true AND `frac != 0`; `false` when `frac == 0` (no roll needed; whole-only scaled) OR `frac != 0` but Bernoulli failed. Consumer derives whole-tickets-awarded as `(preRollTickets / 100) + (roundedUp ? 1 : 0)` and infers consolation as `whole == 0 && preRollTickets > 0` (corroborated by same-tx `LootBoxWwxrpReward`).
- [ ] **LBX-EVT-05**: Threading the index: `_resolveLootboxCommon` signature gains one new parameter `uint48 index` (placed alongside `day` for parameter-order coherence). All 4 callers update: `openLootBox` passes its `index` arg; `openBurnieLootBox` passes its `index` arg; `resolveLootboxDirect` passes `0`; `resolveRedemptionLootbox` passes `0`. Index parameter is consumed ONLY by the `LootboxTicketRoll` emit — no other branches read it. This keeps the non-indexed claim/redemption paths emitting `LootboxTicketRoll` with `lootboxIndex=0` so the indexer schema is uniform; UI can fall back to time/player correlation when index is 0.
...
- **D-274-EVT-ROLL-01**: New `LootboxTicketRoll(address indexed player, uint48 indexed lootboxIndex, uint32 preRollTickets, bool roundedUp)` event. Minimal 4-field schema per user disposition 2026-05-13 — `whole-tickets-awarded` and `consolationPaid` are CONSUMER-DERIVED from `preRollTickets` + `roundedUp` + same-tx `LootBoxWwxrpReward` rather than emitted explicitly. Threads through `_resolveLootboxCommon` via new `uint48 index` parameter; non-indexed claim/redemption paths pass `0` as sentinel
- **D-274-EVT-INDEX-SENTINEL-01**: `lootboxIndex=0` is the sentinel for non-indexed paths (decimator-claim via `resolveLootboxDirect`, sDGNRS-redemption via `resolveRedemptionLootbox`). These paths have no per-index storage slot — consumers fall back to `(player, tx, blockTimestamp)` correlation. Reserving `0` is safe because `openLootBox` / `openBurnieLootBox` lootbox indices start from `1` (or at least never zero in practice; confirm via storage trace during phase plan)
