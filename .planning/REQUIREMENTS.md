...
### LBX-EVT — Event Semantics Rebase + New `LootboxTicketRoll` Event (Phase 274 Wave 1 + Wave 2; backward-incompat break + new event)
...
- [ ] **LBX-EVT-03**: NEW event `LootboxTicketRoll` declared on `IDegenerusGameLootboxModule` (interfaces) + at the LootboxModule contract level near the existing event block (~line 60-110). Signature: `event LootboxTicketRoll(address indexed player, uint48 indexed lootboxIndex, uint32 preRollTickets, bool roundedUp);`. Both `player` and `lootboxIndex` indexed for filter efficiency (per-player + per-lootbox correlation). Emitted exactly once inside `_resolveLootboxCommon` at the end of the `if (futureTickets != 0)` block — after the Bernoulli collapse and after the if/else split that either queues whole tickets or pays consolation. Fires if-and-only-if the ticket-path produced a non-zero pre-Bernoulli scaled value (i.e. only when the outer guard `if (futureTickets != 0)` was entered). Does NOT fire when ticket-path was not selected, or when `_lootboxTicketCount` math truncated scaled to 0 from the start — those cases have no remainder to report.
- [ ] **LBX-EVT-04**: `LootboxTicketRoll` field semantics: `lootboxIndex` is the per-lootbox storage index from `openLootBox(player, index)` / `openBurnieLootBox(player, index)`; `type(uint48).max` (= `0xFFFFFFFFFFFF` = 281,474,976,710,655) for the two non-indexed paths (`resolveLootboxDirect` for decimator claims + `resolveRedemptionLootbox` for sDGNRS redemption) which have no per-index storage slot. Using `type(uint48).max` as sentinel is safe because real lootbox indices are bounded by total lootboxes ever purchased (deep below `2^48 − 1` for any realistic deployment lifetime). `preRollTickets` is the post-distress-bonus, pre-collapse scaled value (i.e. the value snapshotted in LBX-WT-02 as `scaledPre`); divide by `TICKET_SCALE=100` for the floor-tickets, modulo `100` for the round-up Bernoulli weight (e.g. `247` → floor `2`, frac `47`). `roundedUp` is `true` only when the Bernoulli condition `(uint8(seed >> 152) % 100) < uint8(frac)` evaluated true AND `frac != 0`; `false` when `frac == 0` (no roll needed; whole-only scaled) OR `frac != 0` but Bernoulli failed. Consumer derives whole-tickets-awarded as `(preRollTickets / 100) + (roundedUp ? 1 : 0)` and infers consolation as `whole == 0 && preRollTickets > 0` (corroborated by same-tx `LootBoxWwxrpReward`).
- [ ] **LBX-EVT-05**: Threading the index: `_resolveLootboxCommon` signature gains one new parameter `uint48 index` (placed alongside `day` for parameter-order coherence). All 4 callers update: `openLootBox` passes its `index` arg; `openBurnieLootBox` passes its `index` arg; `resolveLootboxDirect` passes `type(uint48).max`; `resolveRedemptionLootbox` passes `type(uint48).max`. Index parameter is consumed ONLY by the `LootboxTicketRoll` emit — no other branches read it. This keeps the non-indexed claim/redemption paths emitting `LootboxTicketRoll` with `lootboxIndex=type(uint48).max` so the indexer schema is uniform; UI filters by sentinel value to classify these as "synthetic" lootbox opens.
- [ ] **LBX-EVT-06**: Event-emission ordering inside the `if (futureTickets != 0)` block (single tx, ticket-path win): `TicketsQueued(player, level, whole)` (or `LootBoxWwxrpReward(...)` on consolation) → `LootboxTicketRoll(player, index, preRollTickets, roundedUp)` → outer `LootBoxOpened(player, day, amount, level, whole, burnie, bonus)` (existing; emitted by the wrapping function at line ~999). Consumers group by `tx` + `player` + `lootboxIndex` to correlate the three events.
...
- [ ] **TST-WT-06**: `LootboxTicketRoll` event emission assertions — four lattice cases (each verified via `(player, lootboxIndex)` topic filter): (a) `preRollTickets=300, frac=0` → emit with `roundedUp=false`, consumer derives whole=3; (b) `preRollTickets=247, Bernoulli wins` → emit with `roundedUp=true`, consumer derives whole=3; (c) `preRollTickets=247, Bernoulli loses` → emit with `roundedUp=false`, consumer derives whole=2; (d) `preRollTickets=47, Bernoulli loses` → emit with `roundedUp=false`, consumer derives whole=0 + correlates same-tx `LootBoxWwxrpReward` for consolation. Plus negative assertion: lootbox open that lands on DGNRS / large-BURNIE / regular-WWXRP path → NO `LootboxTicketRoll` event emitted. Plus zero-scaled assertion: lootbox open where `_lootboxTicketCount` truncated scaled to 0 → NO `LootboxTicketRoll` event emitted (guard not entered). Plus index-correctness: emission from `openLootBox(player, index=42)` carries `lootboxIndex=42`; emission from `resolveLootboxDirect` / `resolveRedemptionLootbox` carries `lootboxIndex=type(uint48).max`.
- [ ] **TST-WT-07**: `LootboxTicketRoll` field-consistency assertions — for each emitted event: derived `whole = (preRollTickets / 100) + (roundedUp ? 1 : 0)` equals same-tx `LootBoxOpened.futureTickets`; `preRollTickets > 0` always (guard); when derived whole `== 0` then same-tx `LootBoxWwxrpReward` exists (consolation correlation); when `lootboxIndex != type(uint48).max` then same-tx storage at `lootboxEth[index][player]` / `lootboxBurnie[index][player]` was zeroed (open completion).
...
- [ ] **AUDIT-03**: §4 adversarial surfaces enumerated for v39.0 changes: (a) EV-neutrality of Bernoulli collapse vs cross-lootbox accumulation; (b) Bit-slice `[152..159]` independence from other primary-chunk consumers; (c) Consolation trigger predicate cannot fire from non-ticket-path; (d) Storage layout byte-identical; (e) Event-shape break documented + no consumer reads stale scaled values; (f) New `LootboxTicketRoll` event field-consistency invariants hold across all 4 callers (`openLootBox` / `openBurnieLootBox` with real index; `resolveLootboxDirect` / `resolveRedemptionLootbox` with `index=type(uint48).max` sentinel); (g) Phase 273 BAF-routing surface coverage at included-baseline.
...
- **D-274-EVT-ROLL-01**: New `LootboxTicketRoll(address indexed player, uint48 indexed lootboxIndex, uint32 preRollTickets, bool roundedUp)` event. Minimal 4-field schema per user disposition 2026-05-13 — `whole-tickets-awarded` and `consolationPaid` are CONSUMER-DERIVED from `preRollTickets` + `roundedUp` + same-tx `LootBoxWwxrpReward` rather than emitted explicitly. Threads through `_resolveLootboxCommon` via new `uint48 index` parameter; non-indexed claim/redemption paths pass `type(uint48).max` as sentinel
- **D-274-EVT-INDEX-SENTINEL-01**: `lootboxIndex=type(uint48).max` (= `0xFFFFFFFFFFFF`) is the sentinel for non-indexed paths (decimator-claim via `resolveLootboxDirect`, sDGNRS-redemption via `resolveRedemptionLootbox`). These paths have no per-index storage slot — consumers fall back to `(player, tx, blockTimestamp)` correlation. Sentinel chosen at the top of the `uint48` domain rather than `0` so it cannot collide with any real lootbox index (real indices grow monotonically from a low base; reaching `2^48 − 1` would require ~281 trillion lifetime lootboxes). User disposition 2026-05-13: `type(uint48).max` selected over `0` to remove the "is index 0 ever real?" verification step from the phase plan
