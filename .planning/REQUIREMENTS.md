...
- [ ] **AGG-02**: The QUEST leg keeps a scheduled ~10-day `mintBurnie` flush that mints the sub's accrued quest BURNIE + applies the ±10 streak, draining `questProgress` once per ~10-day epoch — quests stay AUTOMATIC (the sub's own reward; no sub action). **(amended 2026-06-01 — flat-7% deterministic-split pull; quests stay automatic: the AFFILIATE leg is now PULL with NO scheduled flush; only this quest leg survives as a scheduled flush. See `353-SPEC.md` AFF-01/AGG.)**
