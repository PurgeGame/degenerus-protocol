...
- [x] **FUZZ-03** (FUZZ): GAS-CEILING — no `advanceGame` tx exceeds 16,777,216 gas across any reachable action sequence (TARGETS < 10M — `[[v56-batch-sizing-10m-target-16p7m-ceiling]]`). — DONE 381-03 (REUSABLE AdvanceGasCeilingBase + parameterized GameSeeder in test/gas/AdvanceGasCeiling.sol [Phase 384/COMPO-02 imports it] + AdvanceGasCeilingFuzz.t.sol: 1000-run fuzz over reachable worst-case pre-states, per-tx assertLe(16,777,216), heavy-branch non-vacuity gate, max ~6.6M; v60 game-over composition regression driven through the component max 6.39M < 10M soft target).
- [x] **FUZZ-04** (FUZZ): ENQUEUE — every persisted box (`lootboxEth`/`presaleBoxEth` with base != 0) is in `boxPlayers[index]` until opened (never held un-enqueued — the WHALE-01 invariant, as a fuzz property). — DONE 381-04 (BoxEnqueue.inv.t.sol `invariant_everyPersistedBoxIsEnqueued` over the BoxCreationHandler box-creating action-space [mint-with-lootbox + whale/lazy/deity pass + coin-presale box + an openBoxes/advance+VRF drain]; reuses the WHALE-01 one-shot's BoxQueueViewer etch overlay + boxPlayersContains, extended with presaleBoxBaseFor; asserts every tracked (index,owner) with base!=0 is enqueued, excludes opened boxes base==0; 256 runs / 32768 calls / 0 reverts; non-vacuity across 4 distinct creation paths/16 boxes + a seeded persisted-but-unenqueued falsifiability proof; ZERO contract edits).
- [x] **FUZZ-05** (FUZZ): POOL-CONSERVATION — `futurePrizePool ↔ nextPrizePool ↔ claimablePool` transfers conserve the total; no unbacked credit minted. — DONE 381-05 (PoolConservation.inv.t.sol + PoolFlowHandler; the genuine GAP built case (c) — surpasses the weak existing checks [MultiLevel `invariant_poolSumConsistency` asserts only `balance≥claimablePool`; Composition `invariant_poolSolvency` catches only `sum>balance` after the fact]: `invariant_totalPoolsFullyBacked` = sum(current+next+future+claimable) ≤ balance+stETH [all four pools], `invariant_noUnbackedCreditMinted` = sum(4 pools) ≤ startingBacking + ghost_realInflow [the conservation oracle — an internal future→next→current/skim/jackpot transfer only RESHAPES the split, never mints unbacked credit]; PoolFlowHandler drives buy=real inflow + advance=advanceGame×3+VRF-fulfill [_consolidatePoolsAndRewardJackpots] + claim=real outflow, never vm.stores a pool; 256 runs / 32768 calls / 0 reverts; non-vacuity ghost_advances=6 & sum4pools==realInflow exactly; falsifiability via a seeded unbacked futurePrizePool inflation tripping BOTH bounds; ZERO contract edits).
- [x] **FUZZ-06** (XMODEL): Council reviews the invariant-property SET for COMPLETENESS — feed the council FUZZ-01..05 + the action-space and ask "what property is missing? what reachable sequence violates an unstated invariant?"; fold any convergent gap into the suite before the sweep phases consume it as their oracle. — DONE 381-06: gemini+codex dispatched vs c4d48008; surfaced + REPRODUCED finding V62-01 (permissionless lootbox auto-open structurally dead for human/presale boxes — reads active LR_INDEX, words land at LR_INDEX-1); G1 (free hero-wagers) + G2 (unbounded backfill revert) REFUTED; net-gap NETGAP-02 (advanceGame non-revert liveness) routed to 384/385. Adjudication 381-06-COUNCIL-ADJUDICATION.md; finding in AUDIT-V62-FINDINGS-LEDGER.md.
