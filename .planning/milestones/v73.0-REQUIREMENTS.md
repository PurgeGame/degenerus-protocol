...
- [x] **INV-01**: Activity ROI curve `_roiBpsFromScore` (90→99.9%) unchanged vs HEAD.
- [x] **INV-02**: WWXRP RTP curve `WWXRP_ROI_*` (70→115→118→120%) unchanged vs HEAD.
- [x] **INV-03**: Numeric proof that the WWXRP RTP curve and P(S=9) are unchanged vs HEAD under the rig.
- [x] **INV-04**: The WWXRP S=9 whale-pass bracket award is unchanged vs HEAD.

### TST — verification gates

- [x] **TST-01**: Byte-reproduce gate green — a stat test that regenerates the constants from `derive_5_tables.py` matches the committed constant blocks exactly.
- [x] **TST-02**: `forge build` clean; Degenerette unit + invariant tests + stat oracles green; full-suite parity.
...
- [x] **AUD-01**: After the change (core scoring touched), re-audit the betting engine — Solvency · RNG-integrity (freeze-at-commitment of every WWXRP/Degenerette VRF consumer) · Liveness/no-brick re-attested on the new scoring; cross-model (Codex; gemini if revived) on every load-bearing claim.

### TERM — evidence pack + closure

- [x] **TERM-01**: Evidence pack `audit/FINDINGS-v73.0.md` (+ HTML) + closure signal `MILESTONE_V73_AT_HEAD_<sha>`; subject confirmed byte-frozen at the IMPL diff.
...
| INV-03 | 454 TST | Done |
| INV-04 | 454 TST | Done |
| AUD-01 | 455 REAUDIT | Done |
| TERM-01 | 456 TERMINAL | Done |
