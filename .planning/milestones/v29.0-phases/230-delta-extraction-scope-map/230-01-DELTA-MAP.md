...
Per D-11, this section maps every v29.0 requirement ID (25 total) to the specific sections / row IDs of this document that the downstream consumer (phase 231-236) will cite. Downstream planners enter their scope discovery here — no additional delta-extraction work is required in phases 231-235, and Phase 236 uses this index as its findings-sourcing map.

| Requirement ID | Phase | Section/Row Reference | What the consumer will use |
|---|---|---|---|
| DELTA-01 | 230 | §1 (all 12 subsections §1.1 through §1.12, every function row across all in-scope files) | Self-citation: Phase 230's own evidence that the function-level changelog is complete. Confirms NEW=3, MODIFIED=21, DELETED=0, UNCHANGED=2 totals and all 10 SHA citations. |
| DELTA-02 | 230 | §2 (§2.1 through §2.6, IM-01 through IM-22) | Self-citation: Phase 230's own evidence that every cross-module chain introduced or modified by the delta is catalogued with Caller/Callee/Call Type/Commit SHA/What Changed. |
| DELTA-03 | 230 | §3.1 (ID-01..59), §3.2 (ID-60..71), §3.3 (ID-72..117), §3.4 + §3.5 automated corroboration | Self-citation: Phase 230's own evidence that 117 interface methods each have a PASS/FAIL verdict (all 117 PASS at HEAD) and all four automated gates PASS. |
| EBD-01 | 231 | §1.2 (rows for `_runEarlyBirdLootboxJackpot` MODIFIED by 20a951df is out of scope for EBD-01 — EBD-01 covers f20a2b5e only), §1.4 (`_purchaseFor`, `_callTicketPurchase`), §1.5 (`_purchaseWhaleBundle`, `_purchaseLazyPass`, `_purchaseDeityPass`), §1.6 (`recordMint`), §1.9 (`_awardEarlybirdDgnrs`), §1.1 (`_finalizeRngRequest`, `_finalizeEarlybird`), §2.1 (IM-01..IM-05), commit `f20a2b5e` rows | Phase 231 EBD-01 adversarial audit enumerates every f20a2b5e-touched function + the new level-transition hook and writes a per-function verdict covering CEI, reentrancy, storage ordering, and budget conservation. |
| EBD-02 | 231 | §1.2 (`_runEarlyBirdLootboxJackpot` MODIFIED by 20a951df), §2.3 (IM-16), commit `20a951df` rows | Phase 231 EBD-02 audits the trait-alignment rewrite — bonus-trait parity with coin jackpot, salt-space isolation, fixed-level queueing at `lvl+1`, futurePool → nextPool conservation. |
| EBD-03 | 231 | §1.2 + §1.4 + §1.9 (earlybird surface across Jackpot/Mint/Storage), §2.1 (purchase-phase chains) + §2.3 IM-16 (jackpot-phase trait roll), §1.1 IM-05 (level-transition hook) | Phase 231 EBD-03 traces the combined state machine end-to-end: purchase-phase `_finalizeRngRequest → _finalizeEarlybird` (f20a2b5e) + jackpot-phase `_runEarlyBirdLootboxJackpot` (20a951df) to prove no double-spend, no orphaned reserves, no missed emissions. |
| DCM-01 | 232 | §1.3 (entire file — Decimator module surface), §1.1 (`_consolidatePoolsAndRewardJackpots` MODIFIED by 3ad0f8d3), §1.8 (`BurnieCoin.decimatorBurn` MODIFIED by 3ad0f8d3), §2.2 (IM-06, IM-07, IM-09), commit `3ad0f8d3` rows | Phase 232 DCM-01 audits the key-by-resolution-level refactor: every read site uses `lvl+1` key, pro-rata share math, consolidated jackpot-block ordering. |
| DCM-02 | 232 | §1.3 (`claimDecimatorJackpot`, `claimTerminalDecimatorJackpot` MODIFIED by 67031e7d) + event declarations `DecimatorClaimed`, `TerminalDecimatorClaimed`, §2.2 (IM-08 callee-side), commit `67031e7d` rows | Phase 232 DCM-02 audits event-emission correctness, CEI position, argument correctness, indexer compatibility with v28.0 event surface. |
| DCM-03 | 232 | §1.6 (`DegenerusGame.claimTerminalDecimatorJackpot` NEW by 858d83e4), §1.10 (`IDegenerusGame.claimTerminalDecimatorJackpot` NEW interface decl), §3.1 ID-30, §3.3.d ID-93, §2.2 IM-08, commit `858d83e4` rows | Phase 232 DCM-03 audits the new passthrough: caller restriction, reentrancy, parameter pass-through, no privilege escalation. |
| JKP-01 | 233 | §1.2 (`runBafJackpot` MODIFIED by 104b5d42) + event declarations (JackpotEthWin, JackpotTicketWin widened to uint16) + constant `BAF_TRAIT_SENTINEL = 420`, §2.3 (IM-14, IM-15), commit `104b5d42` rows | Phase 233 JKP-01 audits the BAF sentinel: no collision with real trait IDs (0-255 domain), event-consumer tolerance, no downstream logic treating 420 as a real trait. |
| JKP-02 | 233 | §1.4 (`processFutureTicketBatch` MODIFIED by 52242a10), §1.1 (`_processFutureTicketBatch` / `_prepareFutureTickets` / `advanceGame` MODIFIED by 52242a10), §1.11 (`IDegenerusGameMintModule.processFutureTicketBatch` MODIFIED), §3.3.f ID-103, §2.3 (IM-10..IM-13), §2.5 IM-22, commit `52242a10` rows | Phase 233 JKP-02 audits the entropy passthrough: cryptographic equivalence to prior derivation, no commitment-window widening, no re-use across calls in the same transaction. |
| JKP-03 | 233 | §1.2 (`_runEarlyBirdLootboxJackpot`) + all jackpot-side `bonusTraitsPacked` readers/writers, §2.3 (IM-14..IM-16 — all bonusTrait chains) | Phase 233 JKP-03 verifies every jackpot caller site using `bonusTraitsPacked` produces an identical 4-trait set for the same VRF word across purchase-phase, jackpot-phase, and earlybird paths. |
| QST-01 | 234 | §1.7 (`DegenerusQuests.handlePurchase` MODIFIED by d5284be5), §1.4 (`DegenerusGameMintModule._purchaseFor` + `_callTicketPurchase` MODIFIED by d5284be5), §1.12 (`IDegenerusQuests.handlePurchase` MODIFIED interface), §3.2 ID-67, §2.4 (IM-17, IM-18), commit `d5284be5` rows | Phase 234 QST-01 audits 1:1 wei credit correctness, interaction with fresh-ETH detection, no double-credit, mint-module integration. |
| QST-02 | 234 | §1.9 (`boonPacked` mapping visibility flipped public by e0a7f7bc), §1.10 note on interface non-declaration, §2.4 IM-19, commit `e0a7f7bc` rows | Phase 234 QST-02 audits read-only accessor safety, storage layout preservation, no write path introduced, slot accessibility matches intent. |
| QST-03 | 234 | §1.8 (`BurnieCoin.decimatorBurn` MODIFIED by 3ad0f8d3 — sole BurnieCoin change in delta), §2.2 IM-09, commit `3ad0f8d3` rows | Phase 234 QST-03 audits BurnieCoin change for isolated cause/effect: confined to decimator-burn-key plumbing, no supply-conservation impact. |
| CONS-01 | 235 | §1 (all pool-mutating SSTORE sites — specifically §1.1 `_consolidatePoolsAndRewardJackpots`, §1.2 `_runEarlyBirdLootboxJackpot` + `runBafJackpot`, §1.3 `claimDecimatorJackpot`, §1.4 `_purchaseFor`, §1.6 `recordMint`, §1.1 `_finalizeEarlybird`), §2 (all pool-SSTORE-producing chains), §3 (pool-related interface drift — none at HEAD) | Phase 235 CONS-01 catalogs every SSTORE touching `currentPrizePool` / `nextPrizePool` / `futurePrizePool` / `claimablePool` / `decimatorPool` with mutation direction and guard, then proves sum-before = sum-after at every path endpoint. |
| CONS-02 | 235 | §1.8 (`BurnieCoin.decimatorBurn`), §1.4 (MintModule quest credit chain), §1.7 (`DegenerusQuests.handlePurchase`), §2 (all burn/mint chains crossing the BurnieCoin boundary) | Phase 235 CONS-02 verifies BURNIE conservation: `BurnieCoin.sol` change + quest changes don't break mint/burn accounting; no new mint site bypasses `mintForGame`. |
| RNG-01 | 235 | §1.2 (earlybird bonus-trait roll in `_runEarlyBirdLootboxJackpot`, BAF sentinel emission in `runBafJackpot`), §1.4 (`processFutureTicketBatch` entropy consumer), §1.1 (entropy threading through `_processFutureTicketBatch` / `_prepareFutureTickets`), §2.3 (IM-10..IM-16), §2.5 IM-22 | Phase 235 RNG-01 performs backward trace from every new RNG consumer in the delta to prove the VRF word was unknown at input commitment time (RNG backward-trace methodology per feedback_rng_backward_trace.md). |
| RNG-02 | 235 | §1 (all player-controllable state reads inside new/modified RNG consumers across §1.1, §1.2, §1.4), §2.5 (IM-21, IM-22), §3.4 + §3.5 (automated gate corroboration) | Phase 235 RNG-02 enumerates every player-controllable state variable that can change between VRF request and fulfillment and verifies non-influential for every new consumer (RNG commitment-window methodology per feedback_rng_commitment_window.md). |
| TRNX-01 | 235 | §1.1 (`advanceGame` MODIFIED by 2471f8e7 — `_unlockRng(day)` removed from `JACKPOT_LEVEL_CAP` branch), §2.5 (IM-21 — the deleted call), commit `2471f8e7` rows | Phase 235 TRNX-01 audits the `_unlockRng` removal: RNG lock invariant preserved across the newly-packed housekeeping step, no exploitable state-changing path between `_endPhase()` and the next `_unlockRng` reactivation, no missed or double unlock across normal/gameover/skip-split paths. |
| REG-01 | 236 | §1 (full delta inventory across 12 subsections — regression sweep surface against v27.0 INFO findings + KNOWN-ISSUES) | Phase 236 REG-01 re-verifies all 16 v27.0 INFO findings + 3 KNOWN-ISSUES entries against current code — the §1 inventory is the surface to sweep. |
| REG-02 | 236 | §1 (full delta inventory — regression sweep against v25.0 + v26.0 findings) | Phase 236 REG-02 re-verifies all 13 v25.0 findings + v26.0 delta-audit conclusions against current code — again the §1 inventory is the surface to sweep. |
| FIND-01 | 236 | entire doc (finding-candidate anchors for consolidation — §1 function rows, §2 interaction rows, §3 interface rows) | Phase 236 FIND-01 consolidates all findings from phases 231-235 into `audit/FINDINGS-v29.0.md` with severity / source phase / file:line / resolution status. Every F-29-NN finding cites its source anchor back to a row in this catalog. |
| FIND-02 | 236 | §4 (this index — enables anchoring KNOWN-ISSUES updates to specific F-29-NN IDs during Phase 236) | Phase 236 FIND-02 updates `audit/KNOWN-ISSUES.md` with any new design-decision entries referencing F-29-NN IDs — cross-referenced via this Consumer Index back to the original delta surface. |
| FIND-03 | 236 | entire doc (deliverable source for executive summary table — phase counts, severity counts, requirement-coverage confirmation) | Phase 236 FIND-03 publishes the executive summary table (per-phase counts + per-severity totals) and combined deliverable, sourcing per-requirement per-phase counts from this Consumer Index. |

Requirements covered: 25 / 25

### 4.1 ROADMAP Phase 230 success-criteria confirmation

Per `.planning/ROADMAP.md` Phase 230 section, the four success criteria are:

1. **A function-level changelog lists every changed, added, or deleted function across the 10 commits mapped to owning contract, file, and commit SHA** → §1 (12 subsections covering all 12 in-scope files; every one of the 10 SHAs cited at least once; per-function rows with signature, visibility, verdict, commit SHA, and one-line semantic description; totals = NEW:3, MODIFIED:21, DELETED:0, UNCHANGED:2) — **CONFIRMED**
2. **A cross-module interaction map documents every new or modified call chain that crosses module boundaries within the 12 in-scope files** → §2 (22 IM-NN rows across 6 consumer-phase subsections; five-column format per D-08 with caller, callee, call type, commit SHA, what changed; out-of-scope chains explicitly documented in the preamble) — **CONFIRMED**
3. **An interface-drift catalog compares `IDegenerusGame`, `IDegenerusQuests`, and `IDegenerusGameModules` against their implementers with a PASS/FAIL verdict per signature** → §3 (§3.1 IDegenerusGame 59/59 PASS, §3.2 IDegenerusQuests 12/12 PASS, §3.3 IDegenerusGameModules 46/46 PASS across 9 sub-interfaces; §3.4 + §3.5 automated corroboration records all four gates PASS at HEAD) — **CONFIRMED**
4. **Downstream audit phases (231-234) can reference this phase as their authoritative scope definition with no additional discovery required** → §4 Consumer Index above maps all 25 v29.0 requirement IDs (including Phase 235 CONS/RNG/TRNX and Phase 236 REG/FIND — the acceptance criterion names 231-234 explicitly but this index actually covers 231-236 so no downstream phase has to re-derive scope) — **CONFIRMED**
