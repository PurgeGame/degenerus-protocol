...
1. **[HIGH → RESOLVED 2026-06-01] Accumulator slot premise.** The "Sub spare-bits (176/256)" framing in CONTEXT/PLAN/ROADMAP is inaccurate — the slot is at **232/256 (24 spare)** [VERIFIED: `DegenerusGameStorage.sol:1867-1899`]. ~~The accumulator needs ~192 bits → a new dedicated slot (Option B, §3).~~ **RESOLVED (USER):** fit it IN the slot, NO new cold slot — denominate the affiliate base in WHOLE BURNIE (uint32, 100M saturating clamp) + round `amount` to milli-ETH (uint96→~uint32) + narrow `validThroughLevel`/`lastAutoBoughtDay`/`lastOpenedDay` uint32→uint24, reclaiming well over the ~64 bits the accumulator (`affiliateBase` + `lastSettledDay` + `questProgress`) needs; `windowStartDay` dropped (global ~10-day epoch). GAS-02 re-framed as "one warm per-buy SSTORE into the RE-PACKED Sub slot (no new cold SSTORE)." This is cheaper than Option B (avoids the per-window cold slot). See CONTEXT D-08-accumulator + the 353-01 plan's Accumulator Layout section.
