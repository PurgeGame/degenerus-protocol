...
| K5 | **KEEP-04** — does a *registered* affiliate code exist that routes AfKing-captured revenue to the protocol contract (primary) + VAULT (secondary), and is the capture permanent? | **YES — `bytes32("DGNRS")` (= `AFFILIATE_CODE_DGNRS` :181).** **Owner attribution (corrected vs source :243-250):** the ctor seeds `affiliateCode[AFFILIATE_CODE_VAULT] = { owner: ContractAddresses.VAULT }` (:243-246) and `affiliateCode[AFFILIATE_CODE_DGNRS] = { owner: ContractAddresses.SDGNRS }` (:247-250) — i.e. code `"DGNRS"` is owned by **SDGNRS** (the protocol/"house" contract; distinct from `ContractAddresses.DGNRS`), and code `"VAULT"` is owned by **VAULT**. (An earlier draft of this cell swapped both owners — corrected here.) The ctor then **cross-refers** the two owner addresses: VAULT's own `playerReferralCode` = `"DGNRS"` (:254 → VAULT's upline = SDGNRS); SDGNRS's = `"VAULT"` (:255 → SDGNRS's upline = VAULT). **Routing** under the 75/20/5 winner-takes-all roll (:583-603) for a captured player carrying `"DGNRS"`: **75% → SDGNRS** (owner of `"DGNRS"`, primary) · **20% → upline1 = VAULT** (referrer of SDGNRS) · **5% → upline2 = SDGNRS** (referrer of VAULT). **Permanence:** `_vaultReferralMutable("DGNRS")` (:692-695) is FALSE for any code that is neither `"VAULT"` nor `REF_CODE_LOCKED` → a player whose first purchase carries `"DGNRS"` is locked permanently from purchase #1 (not reclaimable, even during presale); a player already holding a real human code keeps it (`!infoSet` fall-through :463-476). | **RESOLVED: YES — USER-LOCKED 2026-05-25.** Wire `bytes32("DGNRS")` at `DegenerusGame.sol:1778`. It is the **protocol-owned (SDGNRS)** registered code; the two-tier cross-referral routes **primary revenue → SDGNRS (protocol contract), secondary → VAULT** — the user-confirmed intent. **No setup step required.** Do NOT wire `bytes32("VAULT")` (owner VAULT, presale-mutable) — that would send primary to VAULT and be reclaimable during presale. |
| K6 | KEEP-03 foreclosure semantics — `_setReferralCode` :436/:449/:463-476 (unreferred AfKing-joiner permanently captured by VAULT; mutable-default also converts; real human kept) | `DegenerusAffiliate.sol`: `_setReferralCode` body :698-708; foreclosure resolution block :408-476. A blank/invalid referral locks to VAULT (`REF_CODE_LOCKED` :421/:430, referrer→VAULT :702-703). A first valid code stores permanently :436. A player on the MUTABLE VAULT-default is converted to a passed registered code while `_vaultReferralMutable` holds (:449-460; `_vaultReferralMutable` :692-695 = code is REF_CODE_LOCKED/VAULT-default AND `game.lootboxPresaleActiveFlag()` :2244). A player who already holds a real human code: the passed code is ignored (the `!infoSet` fall-through uses the stored code :463-476) | **MATCH** (foreclosure semantics land at :436/:449/:463-476 as claimed; the mutability window is gated by `lootboxPresaleActiveFlag` — relevant to WHEN a blank-first player on the mutable VAULT-default can still be converted to the AfKing-passed `"DGNRS"` code. NOTE: a player whose FIRST purchase is via AfKing locks to `"DGNRS"` immediately and permanently — `"DGNRS"` is immutable always, so the presale window does not apply to that path. Routing is the K5 two-tier (primary SDGNRS / secondary VAULT), NOT "captured by VAULT" directly) |
...
- **KEEP-04: YES — USER-LOCKED 2026-05-25.** Wire the protocol-owned registered code `bytes32("DGNRS")`
  (= `AFFILIATE_CODE_DGNRS`) at `DegenerusGame.sol:1778`. **Owner attribution (corrected vs source
  :243-250):** code `"DGNRS"`→owner **SDGNRS** (the protocol/"house" contract), code `"VAULT"`→owner
  **VAULT** (an earlier draft swapped these). Via the ctor cross-referral (:254-255) + the 75/20/5
  roll (:583-603), an AfKing-captured player on `"DGNRS"` routes **primary (75%) → SDGNRS, secondary
  (20%) → VAULT, 5% → SDGNRS** — the user-confirmed "primary to protocol contract, secondary to
  VAULT" intent. Capture is **permanent from purchase #1** (`_vaultReferralMutable("DGNRS")==false`).
  **No register-one setup step required.** Do NOT wire `bytes32("VAULT")` (owner VAULT, presale-mutable).
