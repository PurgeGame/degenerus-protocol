...
**Success Criteria** (what must be TRUE):
  ...
  5. `autoResolve`'s bounty is re-pegged to a sub-gas flat-per-tx "lose" (GAS-06) — restructured from per-item break-even to ONE flat `creditFlip` per tx gated at ≥5 successfully-resolved NON-WWXRP bets, pegged ETH-equivalent (level-invariant) STRICTLY BELOW the gas to resolve the gate-minimum so every qualifying tx is net-negative (no positive EV at any batch size or under tx-splitting); WWXRP stays excluded (AUTO-04, the gate counts only non-WWXRP), and AUTO-02 probe + per-item isolation + self-resolve-allowed (REW-04) are preserved; `autoResolve` stays a SEPARATE call (the code change rides the BATCH-02 diff, the exact flat amount + gate threshold calibrated here). The SPEC (D-05d) verifies no invariant requires losing-bet resolution before the break-even incentive is dropped.
**Plans**: TBD
**UI hint**: no
...
**Success Criteria** (what must be TRUE):
  ...
  5. The `autoResolve` re-peg is proven (TST-05) — the bounty is flat-per-tx (NOT per-item), the ≥5-resolution gate holds, sub-gas no-positive-EV is proven at any N and under tx-splitting, WWXRP stays excluded from both the gate and the reward, and the resolution RESULTS are byte-identical vs the per-item path (bounty-shape change only, no payout/RNG change).
**Plans**: TBD
**UI hint**: no
...
- **GAS-06 / TST-05** (the `autoResolve` sub-gas-"lose" re-peg, added at the Phase 329 discussion 2026-05-26) → calibrate-at-GAS (331) / prove-at-TST (332); the bounty-logic code change rides the BATCH-02 diff (330). The router-fold of `autoResolve` is OUT (architecturally blocked); only the bounty re-peg is in scope.
...
*v49.0 milestone added: 2026-05-26 (5 phases 329-333, SPEC→IMPL→GAS→TST→TERMINAL; 29 reqs / 7 categories; GAS-06 + TST-05 added at the Phase 329 discussion for the `autoResolve` sub-gas-"lose" re-peg → 31 reqs)*
