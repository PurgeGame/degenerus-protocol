...
**Goal:** Patch `contracts/GNRUS.sol` to (1) reorder `pickCharity` so the queued-edit flush executes AFTER the winner pick + distribution payout (queued edits applied to setCharity during level L take effect for level L+1, not for L itself — closing the queue-branch vote-redirect vector found by independent adversarial agents during Phase 257 Task 7 re-run), and (2) add a `lastWinningRecipient` storage slot updated on payout so that during the next level, `vote(slot)` reverts `PreviousWinnerNotVotable()` if `currentSlate[slot] == lastWinningRecipient` (prevents consecutive wins by the same recipient). Update `test/governance/CharityAllowlist.test.js` to flip the existing "queued replace: voters accumulate against live slot" test to the corrected semantic ("voters pay OLD recipient at level L; queued recipient takes effect at L+1") and add new coverage for the previous-winner vote block. Re-open `audit/FINDINGS-v33.0.md` (currently FINAL READ-only at HEAD `dcb70941`), update §3a delta-surface table + §4 adversarial sweep + §5 regression appendix to reflect the patched semantics, and re-emit the v33.0 closure signal at the new HEAD — superseding `MILESTONE_V33_AT_HEAD_dcb70941`.

**Depends on**: Phase 257 (the Phase 257 independent re-run is what surfaced this bug; Phase 258 fixes it and re-emits closure)

**Requirements**: FIX-01 (pickCharity flush-after-award reorder), FIX-02 (previous-winner vote block), AUDIT-05 (re-audit at patched HEAD; supersede `MILESTONE_V33_AT_HEAD_dcb70941`)

**Success Criteria** (what must be TRUE):
  1. `contracts/GNRUS.sol` `pickCharity(level)` body restructured so flush block (currently lines ~611-630) executes AFTER the distribution payout (currently lines ~668-674); skip-paths A/B/C fall through to flush instead of returning early so queued edits apply to L+1 even on a skipped level.
  2. `contracts/GNRUS.sol` adds `address public lastWinningRecipient` storage slot; `pickCharity` writes `lastWinningRecipient = recipient` after payout (in distribution-paid path only — skip paths leave the value unchanged so a skipped level does not reset the block).
  3. `contracts/GNRUS.sol` adds `error PreviousWinnerNotVotable()` declaration; `vote(uint8 slot)` reverts `PreviousWinnerNotVotable()` if `currentSlate[slot] == lastWinningRecipient` (after existing slot-active and other guards).
  4. `test/governance/CharityAllowlist.test.js` "queued replace" test (currently at lines 305-315) flipped to assert: voters pay the OLD recipient at level L payout; the new recipient appears in `currentSlate` only after `pickCharity(L)` and is votable starting at level L+1 (subject to (5) below if it was the winner).
  5. `test/governance/CharityAllowlist.test.js` new test cases: (a) charity that won level L cannot be voted for at level L+1 via the slot it occupied (revert `PreviousWinnerNotVotable`); (b) if vault-owner queue-replaces the winning slot's recipient between level L payout and level L+1 vote opening, the new recipient is votable at L+1 (because `currentSlate[slot]` no longer equals `lastWinningRecipient`); (c) if level L is skipped (no winner), `lastWinningRecipient` retains its prior value so the L-1 winner remains blocked at L+1.
  6. `audit/FINDINGS-v33.0.md` re-opened (READ-only flag temporarily lifted), §3a delta-surface table updated to add `lastWinningRecipient` (NEW state) + `PreviousWinnerNotVotable` (NEW error) + classify the `pickCharity` reorder as MODIFIED_LOGIC + classify the `vote` guard as MODIFIED_LOGIC; §4 adversarial sweep prose corrected (§4b queue-branch redirect vector now structurally closed by the reorder; surface (a) verdict label re-tagged appropriately given reorder); §5 regression appendix adds REG-01 row confirming v32.0 closure signal still holds at new HEAD; closure signal re-emitted as `MILESTONE_V33_AT_HEAD_<new_sha>` with explicit supersedence note for `MILESTONE_V33_AT_HEAD_dcb70941`. READ-only flip re-applied on terminal commit.
