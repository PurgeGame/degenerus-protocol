...
- [⤴] **Phase 341: TST — FOLDED → v52** (USER 2026-05-28: "move this along and fold tests and shit into v52"). All TST-01..06 proofs (per-tier × per-quadrant, precedence-suppression, revert/dedup, empty-pool no-op, jackpot regression, NON-WIDENING full-suite regression) are consolidated into the v52 audit (cumulative v50+v51 surface). **v52 test note:** the locked revert table referenced `level > currentLevel`, which was REMOVED in 340 IMPL (no level guard — the 8-color ownership check self-gates; signature is `uint24 level`) → the v52 suite MUST drop that revert case and add no-level-guard / uint24 / claimable-near-future-level coverage. Original scope retained for v52 reference: Prove each reward tier pays the correct sDGNRS BPS + BURNIE and marks the correct bits/events; quadrant-first SUPPRESSES the symbol-first bonus AND marks `firstSymbol`; the revert/dedup table (`symbol >= 32` / `gameOver` / non-owner slot / re-claim of a (level,quadrant) / max-4-distinct-quadrants); empty-pool graceful no-op + post-REBAL doubled-pool sizing; jackpot final-day suite green minus the deleted `Pool.Reward` / `JackpotDgnrsWin` assertion; NON-WIDENING full-suite regression vs the v50.0 baseline.
- [⤴] **Phase 342: TERMINAL — FOLDED → v52** (USER 2026-05-28). v51.0 closes at the 340 IMPL HEAD (`c3e9d907` USER-APPROVED); the re-attestation + closure-flip + the already-deferred 3-skill sweep + delta-audit + `audit/FINDINGS-v51.0.md` all consolidate into the v52 consolidated audit (cumulative v50 + v51 surface). Also rides to v52: the one-line 339-BINGO06 freeze-proof re-attestation dropping the `level <= currentLevel` premise (the level guard was removed in IMPL).
...
| 341. TST — Per-Tier + Precedence + Revert/Dedup + Empty-Pool + Jackpot + Non-Widening | v51.0 | — | ⤴ Folded → v52 | 2026-05-28 |
| 342. TERMINAL — Minimal Close: Re-Attest + Closure Flip | v51.0 | — | ⤴ Folded → v52 | 2026-05-28 |
...
*Phase 340 planned: 2026-05-28 (4 plans / 3 waves, every plan autonomous:false — CONTRACT BOUNDARY; producer-before-consumer edit-order enforced via depends_on: W1 340-01 BINGO producers ‖ 340-03 REBAL+JACK → W2 340-02 BINGO consumer wiring → W3 340-04 forge-build + BATCH-02 HARD STOP). 9 reqs: BINGO-01..05 · REBAL-01 · JACK-01/02 · BATCH-02. IMPL bar = forge build clean only per D-340-03; behavior + non-widening regression deferred to 341 TST.*
