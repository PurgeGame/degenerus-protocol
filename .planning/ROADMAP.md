...
- ✅ **v55.0 AfKing-in-Game Redesign** — Phases 348-352 (shipped 2026-06-01; closure signal `MILESTONE_V55_AT_HEAD_ca3bbd3220de763298ef2e742111f6e6ef90d583`; 7 phases incl. 349.1/349.2; 0 NEW_FINDINGS)

---

## ✅ v55.0 AfKing-in-Game Redesign (SHIPPED 2026-06-01)

**Milestone:** v55.0 (started 2026-05-30; SHIPPED 2026-06-01)
**Defined:** 2026-05-30
**Closure signal:** `MILESTONE_V55_AT_HEAD_ca3bbd3220de763298ef2e742111f6e6ef90d583` (resolved at the Phase 352 audit-deliverable / pre-flip HEAD per the v44/v46/v47/v48/v49 sequential-SHA orchestration; contracts byte-identical to the frozen subject `453f8073`; propagated verbatim to FINDINGS + STATE + MILESTONES + PROJECT + REQUIREMENTS).
**Closure verdict (EMITTED, Phase 352):** `AFKING_IN_GAME_FOLD SHIPPED (AfKing.sol dissolved → GameAfkingModule delegatecall + DegenerusGameStorage append, AF_KING address removed 1→0, receive()/credit gates retargeted GAME-only; ARCH-01..04); BOX_REDESIGN SHIPPED (boons-OFF amount=spend, the COMMITTED 4-field stamp scorePlus1+amount+lastAutoBoughtDay+lastOpenedDay [349.1 SUPERSEDED the 348 5-field design], DAY-keyed seed rngWordByDay[lastAutoBoughtDay], live-level open byte-identical to openLootBox, lastOpenedDay no-double-open; BOX-01..05); FREEZE_SPINE INTACT (FREEZE-01/02/03 re-proven against the as-built 4-field/DAY-keyed/live-level model — the live-level collapse NARROWS the frozen set to the SEED, strictly safe); REVERT_FREE_CHAIN DISCHARGED (REVERT-01 _resolveBuy verbatim revert-free-by-construction + REVERT-02 no-valve: fail-loud-on-solvency class B [Panic(0x11)] + terminal-routing-unblocked class C); EVCAP_AT_OPEN (EVCAP-01 RMW exactly-once, buy-time write bypassed, clamp sole live read, equivalent to v54); OPEN-E_4-PROTECTION RE-ATTESTED HOLD (CONSENT-01/02 — HARD BLOCKING condition SATISFIED, closure NOT blocked); SOLVENCY-01 HELD NET (afkingFunding rides inside claimablePool, no new aggregate); GAS Outcome-A (GAS-01/02 structural to the relocation, GAS-03 REJECTED-with-reasoning); RNG_FREEZE_INTACT under the game-resident model; NON-WIDENING 603/134/16 (134-red SUBSET of the v54 148-name union BY NAME); 0 NEW_FINDINGS; KNOWN_ISSUES_UNMODIFIED; + 1 OUT-OF-SCOPE informational advisory O1 [pre-existing symmetric DegenerusQuests lootbox-quest double-credit, NOT in the v55 delta]`. Full report: `audit/FINDINGS-v55.0.md` (chmod 444).
**Audit baseline → subject:** v54 de-custody HEAD `20ca1f79` (v54.0 CLOSED-as-superseded — 343 SPEC + 344 IMPL shipped; 345/346/347 dropped; no ship signal) → v55.0 closure HEAD. Subject FROZEN at `453f8073` (the AfKing-in-Game fold + box redesign — the v54→v55 step IS the AfKing dissolution, 13 contract files differ; `git diff 453f8073 HEAD -- contracts/` empty throughout the terminal); landed across TWO USER-APPROVED contract commits: `77c3d9ef` (349.1 box redesign) + `453f8073` (349.2 quest/affiliate restore). 350 GAS = Outcome A (no contract diff). **Superseded much of v54:** once subscriber state went game-resident, the cross-contract value plumbing + most of the de-custody ledger machinery collapsed into in-context reads. Nothing pushed.
...
> **⚠ Post-execution supersession (this Phase-348 section is the PRE-EXECUTION plan; `348-SPEC-INDEX.md` is authoritative for the locked output).** Three decision overrides landed mid-348 and supersede the narrative below: **D-348-01** — §4 placement = **required-path `advanceGame` STAGE** (not the "leans separate-legs" recommendation in SC3); **D-348-04** — the **try/catch skip valve was DROPPED** → REVERT-02 is the no-valve form (obligation (4) in SC2 below is void); **D-348-07** — the per-sub stamp grew from `(index, amount, day)` → the 5-field `(index, amount, day, scorePlus1, baseLevelPlus1)` (score+baseLevel stamped-frozen, superseding SC1's −EV live-read admission), and the reclaim was re-measured to ~1.4–1.7 KB clean (not SC4's ~2.8 KB). See `348-SPEC-INDEX.md` §5.6 + `348-INVARIANT-CARRY.md`.
...
**Success Criteria** (what must be TRUE):
  ...
  2. The box freeze is RELOCATED into the per-sub stamp, not derived (BOX-01 / BOX-02 / BOX-03) — boons are OFF for afking boxes → box `amount` = spend (the boosted-amount freeze field deleted, BOX-01); the process-pass (pre-RNG) writes the per-sub stamp `(index = current `LR_INDEX`, amount, day, scorePlus1, baseLevelPlus1)` (D-348-07 5-field — score+baseLevel stamped-frozen) as one warm-dirty write per process-day, overwritten each cycle, with NO cold `lootboxEth*`/`lootboxPurchasePacked`/`boxPlayers.push` (BOX-02); and the process-pass debits `afkingFunding` and sets the `lastAutoBoughtDay == today` success-marker ONLY after a successful debit — a failed buy writes no marker (no free box), and a wallet subscribing between the process pass and the open has no this-cycle marker (no free box) (BOX-03).
  ...
  4. The discharged invariants are preserved + the EV-cap lands + the no-brick guarantee is the no-valve form (REVERT-01 / REVERT-02 / EVCAP-01) — the process-pass slice construction preserves `_resolveBuy`'s validation invariants VERBATIM (`ev = cost − claimableUse` + enum payKind, the 1-wei claimable sentinel, the `LOOTBOX_MIN` transient skip, `quantity ≥ 1`) so the funded buy is revert-free by construction (migration fidelity, REVERT-01); the no-brick guarantee is the no-valve form (D-348-04 DROPPED the try/catch valve) — REVERT-01's revert-free-by-construction slice is the sole no-brick guarantor, plus fail-loud-on-solvency (a `claimablePool` underflow MUST revert, never be masked) and terminal-routing-unblocked (the afking STAGE cannot block game-over routing); the two residual revert classes are accepted (solvency-violation [safe under SOLVENCY-01], liveness-timeout [game-dead]) (REVERT-02); and the afking open increments `lootboxEvBenefitUsedByLevel[player][level+1]` via `_applyEvMultiplierWithCap` (read+write-at-open, exactly once per open, same map/key as MintModule's buy-time write, hard-clamped ≤10 ETH → no revert) with the buy-time EV write BYPASSED for afking boxes (no double-draw) (EVCAP-01).
...
**Goal:** The held-349 AfKing box machinery is revised into its final form as one batched `contracts/*.sol` diff (superseding-in-place the held 349 diff): the afking box resolves at the LIVE level (`_rollTargetLevel(level+1, seed)`, mirroring `resolveLootboxDirect`) with its RNG word pinned to the FROZEN stamp day's `rngWordByDay[lastAutoBoughtDay]` and the seed using the FROZEN stamped day (never open-time `_simulatedDayIndex()`); the per-day `_afkingEpoch` epoch (+ pack/unpack) is DROPPED entirely; the process STAGE carries a NO-ORPHAN guard FIRST in the loop (`sub.lastOpenedDay < sub.lastAutoBoughtDay` → skip, dominating all four orphan paths: re-stamp / cancel-reclaim / evict / funding-kill); ticket-mode subs queue whole tickets (no garbage micro-box) while lootbox-mode subs stamp a box (P2 split); the permissionless router `doWork()` is renamed `mintBurnie()`; and the dead `AF_KING` `batchPurchase`/`BatchBuy` + `onlyFlipCreditors` entry are removed — all applied + `forge build` (contracts-only) clean (DegenerusGame < 24,576), then HELD at the contract-commit boundary for explicit USER hand-review. (`forge test` is Phase 351's charge — stale `_afkingEpoch`/ABI/`AfKing.sol`-import test reds are expected until then.)
**Requirements**: No formal REQ-IDs (inserted phase) — must_haves derive from `349.1-DESIGN.md` §7 IMPL checklist + §8 sweep checklist.
**Depends on:** Phase 349 (the held batched diff this phase revises-in-place — applied, contracts-only `forge build` clean, HELD uncommitted at HEAD `60a4b5b5`)
...
- [ ] 349.1-04-PLAN.md — AdvanceModule STAGE caller (drop epochIndex/LR_INDEX) + interface decls + P5 dead-code (batchPurchase/BatchBuy/AF_KING) [wave 4]
- [ ] 349.1-05-PLAN.md — mintBurnie rename (doWork→mintBurnie, atomic) + forge build (contracts-only) clean < 24,576 + HOLD at the contract-commit gate [wave 5; autonomous:false USER contract gate]

### Phase 349.2: IMPL FIX — Restore Quest-Credit + Affiliate for AfKing LOOTBOX Subs (v55 box-redesign regression) (INSERTED)

**Goal:** The v55 box-redesign regression is fixed as one batched `contracts/*.sol` diff (post-349.1 baseline `77c3d9ef`): afking **LOOTBOX-mode** subs again update quest status and pay affiliate like a manual lootbox buy (and like afking TICKET subs already do), WITHOUT re-introducing the cold box-ledger. The redesign made lootbox subs *stamp-only* (`GameAfkingModule.sol:735-749`) to kill the ~120k cold ledger and silently dropped the `quests.handlePurchase` + `affiliate.payAffiliate` those buys used to get via the pre-v55 `batchPurchase` path — the smoking gun is the asymmetry (TICKET subs route through `purchaseWith` and KEPT them; only LOOTBOX subs lost them). Fix: in the LOOTBOX branch of the process STAGE, at the buy/stamp moment, call the lootbox-leg side-effects of `purchaseWith` minus the cold-ledger writes — `quests.handlePurchase(player, spend, 0, spend, priceWei, priceForLevel(level+1))` run BEFORE the score stamp (handlers-before-score → `scorePlus1` from the returned post-buy streak, replacing the pre-action `_questStreakOf` read), plus both `affiliate.payAffiliate` branches (fresh + claimable). KEEP the warm Sub stamp (no cold ledger — GAS-01 preserved), the EV-cap AT OPEN (EVCAP-01 — no buy-time tally), and the TICKET branch unchanged. Affiliate + quest rewards are BURNIE flip-credit (via `coinflip.creditFlip`), NOT an ETH cut (`_ethToBurnieValue` is only the valuation basis), so the existing ETH/`claimablePool` accounting is UNCHANGED — no solvency surface; the IMPL work is faithful replication of `purchaseWith`'s lootbox-leg BURNIE flip-credit accumulation (`coinflip.creditFlip`) + the quest-state update. Applied + `forge build` (contracts-only) clean (DegenerusGame < 24,576), then HELD at the contract-commit boundary for explicit USER hand-review. (`forge test` is Phase 351's charge — stale reds expected until then.) Weekly affiliate aggregation is DEFERRED (not non-gameable without a deterministic-split economic change — see `349.2-DESIGN.md` §5).
**Type:** IMPL (CONTRACT BOUNDARY — one batched USER-APPROVED `contracts/*.sol` diff; `autonomous: false` at the commit gate; never auto-committed)
...
**Depends on**: Phase 349 + Phase 349.2 (the gas pass operates on the post-fold game-resident surface — the warm Sub-stamp write + the in-context subscriber set + the process-batch loop the IMPL established — AND the post-349.2 corrected lootbox path with quest-credit + affiliate restored; GAS-01 is re-confirmed NET OF those restored side-effects, so the gas pass runs on the post-349.2 tree, not the pre-fix surface)
...
**Success Criteria** (what must be TRUE):
  ...
  2. Revert-free + the no-valve no-brick guarantee are proven (TST-02) — a funded process/open never reverts on well-formed slices (exercising the preserved `_resolveBuy` invariants — `ev = cost − claimableUse`, the 1-wei sentinel, the `LOOTBOX_MIN` skip, `quantity ≥ 1`), and the no-valve no-brick guarantee holds (D-348-04 dropped the try/catch): the funded slice is revert-free by construction (REVERT-01) so no funded sub can poison the batch, a solvency violation fails loud (never masked, class B), and game-over routing is never blocked by the afking STAGE (class C).
...
- [x] 352-01-PLAN.md — DELTA-AUDIT: enumerate the 13-file v55 contract delta (vs v54 20ca1f79) NON-WIDENING + the Composition Attestation Matrix (freeze spine [as-built 4-field/DAY-keyed/live-level] + REVERT-FREE-CHAIN + EVCAP-01 + SOLVENCY-01 + OPEN-E 4-protection + VRF-freeze) + the 603/134/16 regression subset-by-NAME (Wave 1)
- [x] 352-02-PLAN.md — ADVERSARIAL-LOG: the 3-skill genuine-PARALLEL sweep (/contract-auditor + /zero-day-hunter + /economic-analyst; /degen-skeptic = dual-gate filter only) on the box-stamp freeze + liveness isolation + two-path open (Wave 1)
...
- [x] 352-04-PLAN.md — CLOSURE FLIP: the single blocking USER gate (autonomous:false) -> resolve the signal SHA `ca3bbd32` + the atomic 5-doc flip + chmod 444 (Wave 3, deps 03) ✅ 2026-06-01
...
**Execution Order:** Phases execute in numeric order: 348 → 349 → 349.1 → 349.2 → 350 → 351 → 352
...
| 352. TERMINAL — Delta Audit + 3-Skill Adversarial Sweep + FINDINGS-v55.0 + Closure | v55.0 | 4/4 | Complete    | 2026-06-01 |
