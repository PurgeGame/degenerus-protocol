...
- **v24.1 Storage Layout Optimization** — Phases 207-212
...
- [x] **Phase 212: Doc Reconciliation** — fix stale SUMMARYs and missing verification after lootboxRngIndex widening (completed 2026-04-10)
...
- [x] 210-04-PLAN.md — Gap closure: fix test file uint8->bool and uint48->uint32 mismatches
- [x] 210-05-PLAN.md — Gap closure: re-run both test suites to confirm zero regressions

### Phase 211: Test Suite Repair
**Goal**: All 113 runtime test failures from v24.1 storage repacking are fixed — both test suites pass with zero new failures
**Depends on**: Phase 210
**Requirements**: VER-02
**Success Criteria** (what must be TRUE):
  1. All vm.load() helpers in Foundry fuzz tests use correct v24.1 slot numbers and bit shifts (slot 38 for lootboxRngPacked, slot 0 byte 28 for ticketWriteSlot)
  2. CompressedJackpot and CompressedAffiliateBonus flag offset assertions use v24.1 slot 0 byte positions
  3. StorageFoundation slot offset assertions match v24.1 layout
  4. Calls to removed setDecimatorAutoRebuy() are removed or replaced
  5. DegenerusStonk-vesting test setup produces level>0 game state
...
- [x] 211-07-PLAN.md — Re-run both test suites, confirm zero failures
...
**Requirements**: TYPE-01, TYPE-02, TYPE-06, SLOT-01, SLOT-02, SLOT-03, SLOT-05, SLOT-06, SLOT-07, SLOT-08
**Success Criteria** (what must be TRUE):
  1. 207-01 SUMMARY correctly describes lootbox-index-keyed mappings as uint48 (not uint32)
  2. 207-02 SUMMARY bit-layout table matches actual lootboxRngPacked implementation (uint48 index at bits 0:47)
  3. 210-01 type audit verdict correctly notes lootboxRngIndex is uint48 in packed slot (not uint32)
...
**Plans**: 1 plan
Plans:
- [x] 212-01-PLAN.md — Fix stale SUMMARYs, update REQUIREMENTS.md, close 208-VERIFICATION gap, document orphaned constants
...
| 212. Doc Reconciliation | 1/1 | Complete   | 2026-04-10 |
