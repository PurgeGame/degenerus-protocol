...
- `LootboxTicketRoll` `lootboxIndex=0` is the sentinel for non-indexed paths (decimator-claim + sDGNRS-redemption). Reserving `0` is safe iff `openLootBox` / `openBurnieLootBox` indices start from `1` — to be confirmed by reading the index-allocation site during phase plan.
