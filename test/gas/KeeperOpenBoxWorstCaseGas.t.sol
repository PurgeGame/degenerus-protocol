...
///             ONLY via `game.mineFlip()` (the open leg at :1000-1009). `game.autoOpen(uint256)` is the
///             HUMAN `boxPlayers` selector (DegenerusGame.sol:1787) and does NOT reach the afking open —
///             so EVERY afking open in this suite is driven by `mineFlip()` after a `_settleClean` (so it
///             routes to OPEN, advance-not-due). N < OPEN_BATCH so one `mineFlip()` opens all N.
///
...
contract KeeperOpenBoxWorstCaseGas is DeployProtocol {
    ...
    ///         the `mineFlip()` open leg gas. Asserts the box is queued (a stamped sub), RNG-ready
    ...
    function testWorstCaseAfkingOpenBoxSingleMaterializationFitsBlockGasLimit() public {
        ...
        // Settle clean so mineFlip routes to the OPEN leg (advance not due), then measure opening exactly
        // ONE queued afking box (the afking open is mineFlip-only; game.autoOpen is the human path).
    ...
    ///         afking open leg (`mineFlip()`) ONCE, divide the gasleft-delta by N so the per-call fixed
    ...
    ///
    ///         The afking open is reached ONLY via `mineFlip()` (the open leg `_autoOpen(OPEN_BATCH)`).
    ...
    function testPerAfkingBoxMarginalAmortizesFixedOverhead() public {
        ...
        // Settle clean so mineFlip routes to OPEN, then bracket the whole afking open leg over the N (+2
        // deploy) ready boxes; divide by the boxes actually opened for the per-box marginal (fixed overhead
        // paid once). N + 2 < OPEN_BATCH so one mineFlip opens them all.
        _settleClean(0x09E20FE);
        assertFalse(game.advanceDue(), "mineFlip routes to OPEN (advance not due)");
        vm.prank(cranker);
        uint256 gasBefore = gasleft();
    ...
    ///      mineFlip routes to OPEN), open ALL of them in one mineFlip, divide by the boxes actually
