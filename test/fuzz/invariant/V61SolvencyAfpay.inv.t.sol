...
///   stale cashout, smite) plus decurse + advance under the [invariant] profile (runs=256, depth=128). Focused
///   non-fuzz scenario tests below additionally prove each named path leaves the identity intact and that
///   curse/smite are pool-neutral (move claimablePool by exactly zero).
///
...
contract V61SolvencyAfpay is DeployProtocol {
    ...
    /// @notice The master SOLVENCY-01 identity across every afking spend path. Reads the raw balancesPacked
    ///         slot for each tracked address and sums both halves; asserts the total equals claimablePool. A
    ///         dropped paired debit (or a double-counted half) would break this.
    function invariant_v61PoolEqualsSumOfHalves() public view {
        address[] memory addrs = handler.trackedAddrs();
        uint256 sum;
