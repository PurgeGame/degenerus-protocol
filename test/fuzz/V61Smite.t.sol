...
/// @title V61Smite — TST-05 proof: deity-smite (the deity adds a curse stack to a smitee for 200 BURNIE),
...
///   then: burnCoin(msg.sender, PRICE_COIN_UNIT/5) = 200 BURNIE; _applyCurseStack(smitee) (+2 saturating at
...
///
///   Every revert leg asserts the caller's BURNIE balance is UNCHANGED — proving the validation is pre-burn
...
///   (tokenId = symbolId 0-31) — distinct from the mintPacked_ HAS_DEITY_PASS score-bonus bit. BURNIE minted
...
contract V61Smite is DeployProtocol {
    ...
    // PRICE_COIN_UNIT = 1000 ether; smite burns PRICE_COIN_UNIT/5 = 200 BURNIE; decurse PRICE_COIN_UNIT/10 = 100.
    ...
    ///         is owned by someone else; the would-be smiter holds 200 BURNIE that must be untouched.
    function testSmiteNonOwnerRevertsNoBurn() public {
        ...
        _fundBurnie(attacker, SMITE_BURN); // funded so a revert is the gate, not insufficient balance

        uint256 burnieBefore = coin.balanceOf(attacker);
        vm.prank(attacker);
        vm.expectRevert();
        game.smite(deityId, smitee);

        assertEq(coin.balanceOf(attacker), burnieBefore, "ownerOf gate: NON-owner caller burned nothing");
    ...
    function testSmiteActiveAfkerImmuneRevertsPreBurn() public {
        ...
        assertEq(coin.balanceOf(deity), burnieBefore, "active-afker immunity: reverts pre-burn (no BURNIE burned)");
    ...
    function testSmiteAtCeilingRevertsNoBurn() public {
        ...
        assertEq(coin.balanceOf(deity), burnieBefore, "ceiling: reverts pre-burn (no BURNIE burned)");
    ...
    function testSmiteBelowCeilingSucceedsThenCeilingBlocks() public {
        ...
        _fundBurnie(deity, SMITE_BURN * 2);
        ...
        uint256 burnieMid = coin.balanceOf(deity);
        vm.prank(deity);
        vm.expectRevert();
        game.smite(deityId, smitee);
        assertEq(game.curseCountOf(smitee), 10, "ceiling blocks the next smite (stays 10)");
        assertEq(coin.balanceOf(deity), burnieMid, "ceiling block: no second burn");
    }

    // =========================================================================
    // Success: burns 200 BURNIE, +1 stack (+2 pts), saturating, emits Smited
    // =========================================================================

    /// @notice A successful smite burns EXACTLY 200 BURNIE (PRICE_COIN_UNIT/5), adds one stack (+2 pts), and
    ...
    function testSmiteSuccessBurns200AddsStackEmits() public {
        ...
        assertEq(burnieBefore - coin.balanceOf(deity), SMITE_BURN, "smite burned EXACTLY 200 BURNIE (PRICE_COIN_UNIT/5)");
        assertEq(coin.balanceOf(deity), 0, "the deity's 200 BURNIE was fully consumed");
    }
    ...
    /// @notice decurse ALSO clears the combined cashout+smite curse for 100 BURNIE — source-agnostic clear via
    ...
    function testDecurseClearsCombinedCashoutAndSmite() public {
        ...
        _fundBurnie(curer, DECURSE_BURN);
        vm.prank(curer);
        game.decurse(smitee);
        assertEq(game.curseCountOf(smitee), 0, "decurse cleared BOTH sources for 100 BURNIE");
    }

    /// @notice Self-smite is allowed (harmless): a deity smiting their OWN address adds a stack to themselves
    ///         and burns 200 BURNIE. Proves the no-anti-self-smite-guard design verdict.
    function testSelfSmiteAllowedHarmless() public {
        ...
        assertEq(burnieBefore - coin.balanceOf(deity), SMITE_BURN, "self-smite burned 200 BURNIE");
    }
