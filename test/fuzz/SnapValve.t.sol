...
contract SnapValveHarness is DegenerusGameMintModule {
    ...
    function exposedSnapOwedPacked(uint80 packed, uint8 s) external pure returns (uint80) {
...
contract SnapValveTest is Test {
    ...
    bytes32 constant ROUND_SIG =
        keccak256("RoundTraitsGenerated(uint24,uint32,uint256,uint32,address[])");

    /// Entries generated by one log: the per-entry event's take, or a round's populated
    /// seat-quadrant count.
    function _entriesOf(Vm.Log memory log) internal pure returns (uint256) {
        if (log.topics[0] == TRAITS_SIG) {
            (, uint32 take) = abi.decode(log.data, (uint256, uint32));
            return take;
        }
        if (log.topics[0] == ROUND_SIG) {
            (, , uint32 mask, ) = abi.decode(log.data, (uint32, uint256, uint32, address[]));
            uint256 c;
            while (mask != 0) {
                c += mask & 1;
                mask >>= 1;
    ...
    function testFuzz_snapFold(uint32 owed, uint8 rem, uint8 s) public view {
        ...
        uint80 packed = (uint80(owed) << 8) | uint80(rem);
        uint80 snapped = h.exposedSnapOwedPacked(packed, s);
    ...
    function test_whaleResumeSingleDivision() public {
        h.seedQueue(LVL, 1, 12000, 0, BASE);
        h.setSnapShift(2);
        address whale = address(BASE + 1);

        (bool finished, ) = h.processTicketBatch(LVL);
        assertFalse(finished, "must span batches");
        uint80 mid = h.owedPacked(LVL, whale);
        assertTrue(mid & h.snapDoneBit() != 0, "marker persists mid-drain");
        assertLt(uint32(mid >> 8), 3000, "partially consumed post-snap balance");
        ...
        uint256 tail;
        for (uint256 i; i < logs.length; ++i) {
            tail += _entriesOf(logs[i]);
        }
        // First batch's take = 12000>>2 minus what the tail batches generated.
        assertEq(h.owedPacked(LVL, whale), 0, "fully drained to zero, no flag residue");
        assertLt(tail, 3000, "first batch did some of the work");
        // Whole-queue conservation: re-seed and drain in one recording for the exact total.
        h.seedQueue(LVL, 1, 12000, 0, BASE + 100);
        uint256 total = _drainAll();
        assertEq(total, 12000 >> 2, "exactly one division across resumes");
    }
