...
contract AdvanceGapBackfillBrick is DeployProtocol {
    ...
    function test_C2_purchasePhaseStall_advanceGameSurvives() public {
        ...
        // Drive only until the gap backfill happens. The purchase clock must move by the exact
        // uncapped gap count once, while dailyIdx remains frozen because STAGE_GAP_BACKFILLED exits
        // before _unlockRng.
        uint24 resumeDay = game.currentDayView();
        uint24 expectedGap = resumeDay - idxBeforeStall - 1;
        bool backfilled;
        ...
        assertEq(_dailyIdx(), idxBeforeStall, "backfill defers before sealing dailyIdx");
        assertTrue(game.rngWordForDay(idxBeforeStall + 1) != 0, "first replay day received a backfill word");
