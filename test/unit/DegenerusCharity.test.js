...
// ContractAddresses constants -- these are compile-time addresses from ContractAddresses.sol.
// The GNRUS test deploys mocks at these addresses using hardhat_setCode.
const ADDR = {
  GAME:       "0x3381cD18e2Fb4dB236BF0525938AB6E43Db0440f",
  SDGNRS:     "0x92a6649Fdcc044DA968d94202465578a9371C7b1",
  VAULT:      "0x796f2974e3C1af763252512dd6d521E9E984726C",
  STETH:      "0x2e234DAe75C793f67A35089C9d99245E1C58470b",
  CREATOR:    "0x7FA9385bE102ac3EAc297483Dd6233D62b3e1496",
};
...
// ---------------------------------------------------------------------------
// Helper: deploy a mock contract, then copy its bytecode to targetAddress
// ---------------------------------------------------------------------------
...
async function deployCharityFixture() {
  ...
  const [deployer, voter1, voter2, voter3, recipient1, recipient2, recipient3, ...others] = signers;

  // Deploy mocks at the ContractAddresses addresses
  const mockSdgnrs = await deployMockAt("MockSDGNRSCharity", ADDR.SDGNRS);
  const mockGame   = await deployMockAt("MockGameCharity", ADDR.GAME);
  const mockVault  = await deployMockAt("MockVaultCharity", ADDR.VAULT);
  const mockSteth  = await deployMockAt("MockStETH", ADDR.STETH);
  ...
  // Give voter1 1% (above 0.5% threshold)
...
// ---------------------------------------------------------------------------
async function distributeGNRUS(charity, mockVault, mockSdgnrs, voter, recipientAddr) {
...
describe("GNRUS (GNRUS)", function () {
  ...
  describe("Token Metadata", function () {
    it("name is 'Degenerus Donations'", async function () {
      ...
      expect(await charity.name()).to.equal("Degenerus Donations");
    });
  ...
  describe("Burn Redemption", function () {
    ...
    it("burn reduces totalSupply by burned amount", async function () {
      const { charity, mockVault, mockSdgnrs, voter1, recipient1 } = await loadFixture(deployCharityFixture);
      // Distribute GNRUS to recipient1 via governance
      await distributeGNRUS(charity, mockVault, mockSdgnrs, voter1, recipient1.address);
    ...
    it("burn with ETH backing pays proportional ETH", async function () {
      const { charity, charityAddress, mockVault, mockSdgnrs, voter1, recipient1, deployer } = await loadFixture(deployCharityFixture);
    ...
    it("burn with stETH backing pays proportional stETH", async function () {
      const { charity, charityAddress, mockVault, mockSdgnrs, mockSteth, voter1, recipient1, deployer } = await loadFixture(deployCharityFixture);
    ...
    it("burn with claimable winnings: lazy claim pulls from game when on-hand insufficient", async function () {
      const { charity, charityAddress, mockVault, mockSdgnrs, mockGame, voter1, recipient1, deployer } = await loadFixture(deployCharityFixture);
      ...
      await deployer.sendTransaction({ to: ADDR.GAME, value: claimableAmount });
  ...
  describe("Governance -- Propose", function () {
    ...
    it("propose snapshots sDGNRS totalSupply on first proposal", async function () {
      ...
      await charity.connect(voter1).propose(recipient1.address);
      expect(await charity.levelSdgnrsSnapshot(0)).to.equal(sdgnrsSupply);
    });
  ...
  describe("Governance -- Vote", function () {
    it("vote(proposalId, true) increases approveWeight by voter's sDGNRS balance", async function () {
      ...
      expect(ev.args.weight).to.equal(voter2Bal);

      const [, , approveWeight] = await charity.getProposal(0);
      expect(approveWeight).to.equal(voter2Bal);
    });
    ...
    it("vote(proposalId, false) increases rejectWeight by voter's sDGNRS balance", async function () {
      ...
      expect(rejectWeight).to.equal(voter2Bal);
    });
  ...
  describe("Governance -- pickCharity", function () {
    ...
    it("2% decay: second level distributes less than first", async function () {
      const { charity, charityAddress, mockVault, mockSdgnrs, voter1, recipient1, recipient2 } = await loadFixture(deployCharityFixture);
  ...
  describe("Edge Cases", function () {
    it("multiple levels: proposals from previous levels are not accessible for voting", async function () {
      const { charity, mockVault, voter1, recipient1, recipient2 } = await loadFixture(deployCharityFixture);
      await mockVault.setVaultOwner(voter1.address, true);
    ...
    it("community proposer can propose in new level after resolve", async function () {
      const { charity, mockVault, voter1, voter2, recipient1, recipient2 } = await loadFixture(deployCharityFixture);
    ...
    it("totalSupply is conserved: unallocated + all holders = totalSupply", async function () {
      ...
      await distributeGNRUS(charity, mockVault, mockSdgnrs, voter1, recipient1.address);
      await distributeGNRUS(charity, mockVault, mockSdgnrs, voter1, recipient2.address);
    ...
    it("vault owner snapshot locks on first vault-owner action per level", async function () {
      ...
      const tx = await charity.connect(voter2).vote(0, true);
      const ev = await getEvent(tx, charity, "Voted");
      const voter2Bal = await (await hre.ethers.getContractAt("MockSDGNRSCharity", ADDR.SDGNRS)).balanceOf(voter2.address);
      // Weight should equal voter2's sDGNRS balance without bonus
      expect(ev.args.weight).to.equal(voter2Bal);
    });
  });
});
