...
contract DegenerusGameAdvanceModule is DegenerusGameStorage {
    ...
    uint96 private constant MIN_LINK_FOR_LOOTBOX_RNG = 40 ether; // LINK, not ETH

    /// @dev Presale auto-ends after this much mint-only lootbox ETH (200 ETH, unscaled).
    uint256 private constant LOOTBOX_PRESALE_ETH_CAP = 200 ether / D;
    ...
    function rngGate(
        ...
        if (rngRequestTime != 0) {
            ...
            if (elapsed >= 5 minutes) { // TESTNET: reduced from 18 hours
                _requestRng(isTicketJackpotDay, lvl);
                return 1;
            }
            revert RngNotReady();
    ...
    function _finalizeRngRequest(
        ...
        if (isTicketJackpotDay && !isRetry) {
            ...
            if (lvl == 5) {
                price = uint128(0.02 ether / D);
            } else if (lvl == 10) {
                price = uint128(0.04 ether / D);
            } else if (lvl == 30) {
                price = uint128(0.08 ether / D);
            } else if (lvl == 60) {
                price = uint128(0.12 ether / D);
            } else if (lvl == 100) {
                price = uint128(0.24 ether / D);
            } else if (lvl > 100) {
