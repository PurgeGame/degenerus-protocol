...
# -------------------------------------------------------------------------
# WWXRP-only payout SHAPE (the rigged lane).
# -------------------------------------------------------------------------
# The rigged lane carries its own shape because its win profile is different: the
# M>=2 rig band leaves P(S=2) at roughly its honest value but makes S=3/S=4/S=5
# several times more common, so the honest SHAPE solved against that distribution
# lands S=2 well under the stake (a sub-1x "win"). SHAPE_RIG pins S=2 to a
# money-back push (1.00x at the flat 70% floor) and keeps every tier above it a
# real multiple. The mid tiers are deliberately modest and the top ratios well
# below the honest SHAPE's: the rig makes S=3..S=5 frequent, so paying them
# generously would starve S=6..8 and flatten the ladder. Both shapes still solve
# to basePayoutEV = 100 centi-x, and S=9 stays on the shared pin.
SHAPE_RIG = [0, 0, 143, 260, 500, 1100, 2706, 13879, 693_933, None]  # S=9 pinned

print(f"Payout SHAPE (relative, S=2..8): {SHAPE[2:9]}  (S=9 pinned to the M=8 relabel)")
print(f"WWXRP RIG SHAPE (relative, S=2..8): {SHAPE_RIG[2:9]}  (S=9 shares the pin)\n")
...
def _solve_table(pS, N, shape=SHAPE):
    ...
    nudge (never EV-positive). `shape` defaults to the honest SHAPE; the WWXRP
    rigged family passes SHAPE_RIG."""
...
# ===================================================================
# WWXRP RIG FAMILY — score-bearing rig (+1, 60%, 2 <= M <= 6 band).
# Under Variant-2 the WWXRP rig forces ONE *score-bearing* cell to a real
# match w.p. 3/5 when 2 <= M <= 6: an unmatched NON-HERO symbol (+1), or an
...
# rig can NEVER manufacture S=9 -> P(S=9) is INVARIANT (RIG-02); floors at
# M=2 so it never manufactures the S=2 dribble tier out of a near-empty reel,
# which leaves P(S=2) near its honest value while P(S>=4) keeps the FULL lift of
# an ungated rig (reaching S>=4 post-force always started at M>=2). And
...
def p_score_distribution_rigged(n_gold):
    ...
    roll has 2 <= M <= 6 matched axes. M >= 7 (full match / single miss) is untouched
    so the rig can NEVER manufacture (or destroy) S=9 -> P(S=9) is invariant (RIG-02).
    M < 2 is untouched so the rig never manufactures the S=2 dribble tier out of a
    near-empty reel. The floor costs nothing above it: reaching S >= 4 post-force always
    started at M >= 2, so P(S>=4) and P(S>=6) are IDENTICAL to an ungated rig, while
    P(S=2) stays near its honest value instead of being inflated by ~9 points.
    ...
    fired roll has 2<=M<=6 -> post-force M<=7 -> S<=8, so a +2 can NEVER reach S=9
    ("not if it makes a 9/9"); the rig[9]==honest[9] assert is the hard proof.

    Explicit EMPTY-ELIGIBLE-POOL case: when 2 <= M <= 6 but every unmatched cell is an
    ...
    for hero_is_gold, weight, ord_gold, ord_common in (
        ...
        for (S, M, e1, e2), p in partials.items():
            etot = e1 + e2
            if M >= 7 or M < 2:
                # m>=7 cap: untouched -> P(S=9) invariant.
                # m<2 floor: a near-empty reel (0 or 1 matched axes) is left alone, so
                # the rig never manufactures the S=2 dribble tier out of nothing.
                # Everything from S=4 up keeps the FULL lift of an ungated rig, because
                # reaching those post-rig always started at M>=2 anyway.
                out[S] += p
            elif etot == 0:
...
# for the averaged WWXRP _RIG_ family — same S9 pin / residual solve, but against
# SHAPE_RIG so the smallest WWXRP win is a money-back push rather than a sub-1x dribble.
tables_rig = [_solve_table(P_N_RIG[N], N, SHAPE_RIG) for N in range(5)]
