...
for row in tokens:
    ...
    color = []
    if tokenId >30000:
        tokenIdfilename="MAPImages" + os.sep + str(tokenId) + '.png'
    else:
        tokenIdfilename = "finished" + os.sep +str(tokenId) + '.png'
    for c in range(1,5):
        cur.execute("""
        SELECT color,shape
        FROM traits
    ...
    smaller.save("smalltokens" + os.sep +str(tokenId) + '.png')

bomb = Image.open('baseimages' + os.sep + "bomb.png")
bomb = bomb.convert("RGBA")
background = Image.open("baseimages" + os.sep + "background" + colorfound + ".png")
Image.Image.paste(background,bomb, (0,0),bomb)
background.save('finished' + os.sep + 'bomb.png')
background.save('smalltokens' + os.sep + 'bomb.png')
print('done')