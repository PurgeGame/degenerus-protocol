...
- **`forge test`** — **the Foundry suite: 2,053 passing, 104 skipped, 2 pre-existing stale fixtures** (`SweepWorstCaseDrain` and `V56QuestNonPerturb`, standing test debt predating this freeze). Coverage spans unit, integration, fuzz, invariant, gas, access-control, governance, economics, and named regression harnesses for every fixed finding.
