...
- All contracts under the 24,576-byte EIP-170 limit (largest: DegenerusGame at 24,373 bytes, 203 to spare; AdvanceModule at 24,053, 523 to spare; MintModule at 23,986, 590 to spare)
...
- **`forge test`** — **1,465 Foundry tests across 198 suites**, all passing: unit, integration, fuzz, invariant, gas, access-control, governance, economics, and named regression harnesses for every fixed finding.
...
- **`scope.txt` / `out_of_scope.txt`** — the exact audited surface, pinned to `contracts/` tree `0e7b02fe` (tag `degenerus-c4a`).
