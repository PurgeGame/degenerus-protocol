...
- All deployed contracts under the 24,576-byte EIP-170 limit (largest: MintModule at 24,444 bytes, 132 to spare; AdvanceModule at 24,360, 216 to spare; DegenerusGame at 24,134, 442 to spare; CrapsBattle at 23,752, 824 to spare — its dice engine now a 3,973-byte contract of its own)
...
- **Growth parimutuel:** an over/under book on the ratchet the game already records. Round L, open through level L's whole jackpot phase, resolves OVER iff `growth(L+1) > growth(L)` where `growth(L) = ratchet(L)/ratchet(L-1)` — compared cross-multiplied so the arithmetic is exact, with a push resolving UNDER. One fixed 1,000-FLIP stake per address per round, burned at placement; winners split the entire book evenly through the coinflip credit rail. Settlement is pushed, not derived: the level transition that banks the successor ratchet entry calls `recordGrowth` and emits `GrowthRoundSealed`, latching the side into a write-once two-bit outcome (century levels scored against the append-only achieved-pool history). Claims read that bit, so a settled round's answer can never change.
...
- **Whale Pricing:** Bundles 2.4-4 ETH, lazy passes 0.24 ETH+, deity passes 24 + T(n) ETH triangular through the 27th (375 ETH), then doubling each pass to 12,000 ETH for the 32nd. A bundle buys one pass per unit and awards one extra pass's entries per 5 bought in the same call; its 10% lootbox lands as one custom box per pass bought, or as fewer larger boxes as the 100-box entry cap closes, with any customs already held in that index folded in at the value-weighted average size (a full entry folds the value into its held customs, and reverts only when it has none; no entry ever holds more than 100 boxes), and price, lootbox, DGNRS, affiliate and Craps credit follow the paid quantity only.
...
- **Source-drift gates** (`make check-*`) — interface coverage, delegatecall target alignment, raw-selector bans, RNG-window consumer classification, RNG-word taint (every function that receives a word by parameter and every cross-contract call that passes one, `scripts/rng-taint-manifest.tsv`), advance-chain external calls (every external call on the daily crank's reachable set classified, and every bare crank call pinned to the test that drives its callee, `scripts/advance-call-manifest.tsv`), pool-write provenance, unbounded storage-array deletes, craps progressive parity, the storage write-ownership registry (which file may write which shared storage variable, `scripts/write-owners-manifest.tsv`), and the unchecked-arithmetic registry (every `unchecked` block in the game facade, storage base and modules recorded with the invariant that keeps it in range, `scripts/unchecked-manifest.tsv`).
- **Static analysis** — Slither + Aderyn (non-blocking).
- **Weekly** — Halmos symbolic proofs (`FOUNDRY_PROFILE=halmos`, which compiles only the symbolic suites under via-IR; the craps arithmetic set needs `--loop 8`) + a deep invariant sweep (runs=1000, depth=256).

Reproduce the core suite locally:

```
forge test    # 2,293 passing, 104 skipped
...
- **`scope.txt` / `out_of_scope.txt`** — the exact audited surface, pinned to `contracts/` tree `58367471` (tag `degenerus-c4a`).
