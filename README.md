...
- All contracts under the 24,576-byte EIP-170 limit (largest: DegenerusGame at 24,472 bytes, 104 to spare; AdvanceModule at 24,440, 136 to spare; MintModule at 24,435, 141 to spare)
...
- **`forge test`** — **1,506 Foundry tests across 201 suites**, all passing: unit, integration, fuzz, invariant, gas, access-control, governance, economics, and named regression harnesses for every fixed finding.
...
- **`scope.txt` / `out_of_scope.txt`** — the exact audited surface, pinned to `contracts/` tree `4018e0d4` (tag `degenerus-c4a`).
