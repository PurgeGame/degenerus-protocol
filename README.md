...
- All deployed contracts under the 24,576-byte EIP-170 limit (largest: DegenerusGame at 24,487 bytes, 89 to spare; CrapsBattle at 24,435, 141 to spare; AdvanceModule at 24,039, 537 to spare)
...
- **`forge test`** — **the Foundry suite: 2,011 passing, 104 skipped, 1 pre-existing stale fixture** (`SweepWorstCaseDrain`, standing test debt unrelated to this tree). Coverage spans unit, integration, fuzz, invariant, gas, access-control, governance, economics, and named regression harnesses for every fixed finding.
...
```
forge test    # 2,011 passing, 104 skipped, 1 known-stale fixture
...
- **`scope.txt` / `out_of_scope.txt`** — the exact audited surface, pinned to `contracts/` tree `09413eb0` (tag `degenerus-c4a`).
