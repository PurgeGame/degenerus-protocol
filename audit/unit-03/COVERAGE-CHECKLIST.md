...
|---|----------|-------|----------|----------|----------------|-------|-----------|------------|--------------|-------------|
| C1 | `_runEarlyBirdLootboxJackpot()` | 772-835 | JackpotModule | B2 | futurePrizePool (via _setFuturePrizePool L778), nextPrizePool (via _setNextPrizePool L834), ticketQueue (via _queueTickets L819) | | pending | pending | pending | pending |
| C2 | `_distributeYieldSurplus()` | 883-914 | JackpotModule | B5 | claimableWinnings (via C3 for VAULT, SDGNRS addresses), claimablePool (L911), yieldAccumulator (L913) | [BAF-PATH] | pending | pending | pending | pending |
| C3 | `_addClaimableEth()` | 928-949 | JackpotModule | C2, C11, C14 (via C15), C16 (via C15), C12 (via C13->C14->C15/C16) | claimableWinnings[beneficiary] (via C26 _creditClaimable L947), OR futurePrizePool/nextPrizePool (via C4 _processAutoRebuy L982-984), ticketQueue (via C4->_queueTickets L979), claimableWinnings[player] (via C4->C26 L988) | **[MULTI-PARENT] [BAF-CRITICAL]** | pending | pending | pending | pending |
| C4 | `_processAutoRebuy()` | 959-999 | JackpotModule | C3 | futurePrizePool (via _setFuturePrizePool L982) or nextPrizePool (via _setNextPrizePool L984), ticketQueue (via _queueTickets L979), claimableWinnings[player] (via C26 _creditClaimable L988) | **[BAF-CRITICAL]** | pending | pending | pending | pending |
| C5 | `_distributeLootboxAndTickets()` | 1050-1073 | JackpotModule | B2 | nextPrizePool (via _setNextPrizePool L1058), ticketQueue (via C6->C7->C8->_queueTickets) | | pending | pending | pending | pending |
| C6 | `_distributeTicketJackpot()` | 1076-1109 | JackpotModule | C5, B3 | ticketQueue (via C7->C8->_queueTickets) | **[MULTI-PARENT]** | pending | pending | pending | pending |
| C7 | `_distributeTicketsToBuckets()` | 1112-1146 | JackpotModule | C6 | ticketQueue (via C8->_queueTickets) | | pending | pending | pending | pending |
| C8 | `_distributeTicketsToBucket()` | 1149-1190 | JackpotModule | C7 | ticketQueue (via _queueTickets L1180) | | pending | pending | pending | pending |
| C9 | `_executeJackpot()` | 1280-1294 | JackpotModule | B2 (early-burn path) | claimableWinnings, claimablePool (via C10->C12->C13->C14->C3) | | pending | pending | pending | pending |
| C10 | `_runJackpotEthFlow()` | 1297-1322 | JackpotModule | C9 | claimableWinnings, claimablePool (via C12) | | pending | pending | pending | pending |
| C11 | `_processDailyEth()` | 1338-1433 | JackpotModule | B2 (daily Phase 0 and Phase 1) | claimableWinnings[winners] (via C3 L1407), claimablePool (L1430) | **[BAF-PATH]** | pending | pending | pending | pending |
| C12 | `_distributeJackpotEth()` | 1435-1474 | JackpotModule | B1, C10 | claimableWinnings (via C13->C14->C3), claimablePool (L1471), whalePassClaims (via C14->C16), futurePrizePool (via C14->C16->_setFuturePrizePool, C14->C3->C4) | **[MULTI-PARENT] [BAF-PATH]** | pending | pending | pending | pending |
| C13 | `_processOneBucket()` | 1477-1504 | JackpotModule | C12 | claimableWinnings (via C14->C3), whalePassClaims (via C14->C16), futurePrizePool (via C14->C16, C14->C3->C4) | | pending | pending | pending | pending |
| C14 | `_resolveTraitWinners()` | 1528-1655 | JackpotModule | C13 | claimableWinnings (via C3 L1627, via C16->C15->C3), claimablePool (aggregated by caller), whalePassClaims (via C16 L1709), futurePrizePool (via C16 L1710, via C3->C4) | **[BAF-PATH]** | pending | pending | pending | pending |
| C15 | `_creditJackpot()` | 1663-1676 | JackpotModule | C14 | claimableWinnings (via C3 if ETH path L1674) or coin.creditFlip (if coin path L1670, external) | | pending | pending | pending | pending |
| C16 | `_processSoloBucketWinner()` | 1684-1717 | JackpotModule | C14 | claimableWinnings (via C15->C3 L1706/L1714), whalePassClaims[winner] (L1709), futurePrizePool (via _setFuturePrizePool L1710) | **[BAF-PATH]** | pending | pending | pending | pending |
| C17 | `_resolveZeroOwedRemainder()` | 1877-1904 | JackpotModule | C18 | ticketsOwedPacked[rk][player] (L1888, L1895, L1901) | | pending | pending | pending | pending |
| C18 | `_processOneTicketEntry()` | 1907-1970 | JackpotModule | B6 | ticketsOwedPacked[rk][player] (via C17, C21), traitBurnTicket[lvl][traitId] (via C19->C20 assembly) | | pending | pending | pending | pending |
| C19 | `_generateTicketBatch()` | 1973-1993 | JackpotModule | C18 | traitBurnTicket[lvl][traitId] (via C20 assembly SSTORE) | | pending | pending | pending | pending |
| C20 | `_raritySymbolBatch()` | 2050-2145 | JackpotModule | C19 | traitBurnTicket[lvl][traitId] -- manual slot math via inline Yul: keccak256(lvl, traitBurnTicket.slot) + traitId for array length, keccak256(elem) for data start. Raw SSTORE of player address. | **[ASSEMBLY]** | pending | pending | pending | pending |
| C21 | `_finalizeTicketEntry()` | 1996-2021 | JackpotModule | C18 | ticketsOwedPacked[rk][player] (L2018) | | pending | pending | pending | pending |
| C22 | `_awardDailyCoinToTraitWinners()` | 2341-2438 | JackpotModule | B3, B7 | none (external calls only: coin.creditFlipBatch L2411/L2437, coin.creditFlip via batch) | **[MULTI-PARENT]** | pending | pending | pending | pending |
| C23 | `_awardFarFutureCoinJackpot()` | 2444-2529 | JackpotModule | B3, B7 | none (external calls only: coin.creditFlipBatch L2510/L2527) | **[MULTI-PARENT]** | pending | pending | pending | pending |
| C24 | `_syncDailyWinningTraits()` | 2550-2558 | JackpotModule | B2, B7 | lastDailyJackpotWinningTraits (L2555), lastDailyJackpotLevel (L2556), lastDailyJackpotDay (L2557) | **[MULTI-PARENT]** | pending | pending | pending | pending |
| C25 | `_clearDailyEthState()` | 2708-2714 | JackpotModule | B2 | dailyEthPhase=0 (L2709), dailyEthPoolBudget=0 (L2710), dailyCarryoverEthPool=0 (L2711), dailyCarryoverWinnerCap=0 (L2712), dailyJackpotCoinTicketsPending=true (L2713) | | pending | pending | pending | pending |
| C26 | `_creditClaimable()` | PU:30-36 | PayoutUtils | C3, C4 | claimableWinnings[beneficiary] (PU:33) | | pending | pending | pending | pending |
| C27 | `_queueWhalePassClaimCore()` | PU:75-91 | PayoutUtils | (not called directly from JackpotModule; available for EndgameModule) | whalePassClaims[winner] (PU:82), claimableWinnings[winner] (PU:86), claimablePool (PU:88) | | pending | pending | pending | pending |
| C28 | `_creditDgnrsCoinflip()` | 2269-2275 | JackpotModule | B5 | none (external call: coin.creditFlip L2274) | | pending | pending | pending | pending |
...
|---|----------|-------|---------------|---------------|-----------|-----------|
| D1 | `_hasTraitTickets()` | 1002-1021 | `traitBurnTicket[lvl][trait].length`, `deityBySymbol[fullSymId]` | View: checks if any packed traits have real tickets or virtual deity entries. Deity virtual entry gate: `fullSymId < 32 && deityBySymbol[fullSymId] != address(0)`. | ETH-DIST / TICKET-DIST | pending |
| D2 | `_budgetToTicketUnits()` | 1034-1041 | `PriceLookupLib.priceForLevel(lvl)` | Pure: converts ETH budget to ticket units via `(budget << 2) / ticketPrice`. Returns 0 if ticketPrice is 0. No overflow risk (Solidity 0.8.34). | TICKET-DIST | pending |
| D3 | `_computeBucketCounts()` | 1193-1245 | `traitBurnTicket[lvl][trait].length`, `deityBySymbol[fullSymId]` | View: computes winner counts for active trait buckets with even distribution + entropy-based remainder. | ETH-DIST / TICKET-DIST | pending |
| D4 | `_futureKeepBps()` | 1252-1267 | Pure: `keccak256(rngWord, FUTURE_KEEP_TAG)` | Pure: 5-dice roll with [0-3] range each, mapped to 3000-5333 BPS (30-53.3%). Range bounds: total in [0,15], BPS in [3000, 3000+15*3500/15] = [3000, 6500]. | POOL-MGMT | pending |
...
| D6 | `_topHeroSymbol()` | 1767-1795 | `dailyHeroWagers[day][q]` -- packed 32-bit amounts per symbol | View: finds top hero symbol by wagered amount. Iterates 4 quadrants x 8 symbols = 32 entries. Deterministic tie-break: first seen (q asc, symbol asc). | TRAIT-SELECTION | pending |
| D7 | `_randTraitTicket()` | 2160-2203 | `traitBurnTicket_[trait]`, `deityBySymbol[fullSymId]` | View: selects random winners via `slice % effectiveLen`. Duplicates intentionally allowed. Virtual deity entries: `floor(len/50)`, minimum 2. **RNG note:** Bit rotation (16-bit shift) for subsequent winners -- not cryptographically independent but acceptable for jackpot selection. **Modulo bias:** present if effectiveLen not power-of-2 (known/intentional per docs). | WINNER-SELECTION | pending |
| D8 | `_randTraitTicketWithIndices()` | 2206-2260 | Same as D7 plus `ticketIndexes` output | View: same selection as D7 plus ticket indices for event emission. Deity entries get `ticketIndex = type(uint256).max`. Same RNG note as D7. | WINNER-SELECTION | pending |
| D9 | `_calculateDayIndex()` | 2265-2267 | Delegates to `_simulatedDayIndex()` (inherited from DegenerusGameStorage) | View: day index calculation. Single wrapper. | TIMING | pending |
| D10 | `_rollWinningTraits()` | 2533-2548 | `JackpotBucketLib.packWinningTraits()`, `_getWinningTraits()`, `JackpotBucketLib.getRandomTraits()` | View: rolls or derives packed winning traits. `useBurnCounts=true` uses hero override (D5). `useBurnCounts=false` uses fully random traits. `lvl` parameter is unused (assigned to itself, dead code). | TRAIT-SELECTION | pending |
| D11 | `_loadDailyWinningTraits()` | 2560-2567 | `lastDailyJackpotWinningTraits`, `lastDailyJackpotDay`, `lastDailyJackpotLevel` | View: loads stored daily winning traits. Validity check: day and level must match current values. | STATE-CACHE | pending |
| D12 | `_calcDailyCoinBudget()` | 2570-2574 | `price`, `levelPrizePool[lvl-1]` | View: calculates 0.5% of prize pool target in BURNIE. Formula: `(levelPrizePool[lvl-1] * PRICE_COIN_UNIT) / (price * 200)`. Returns 0 if price is 0. **Note:** Uses `lvl-1` for prize pool lookup (previous level's pool). | COIN-JACKPOT | pending |
| D13 | `_dailyCurrentPoolBps()` | 2579-2592 | Pure: `keccak256(randWord, DAILY_CURRENT_BPS_TAG, counter)` | Pure: days 1-4 random 6%-14%, day 5+ returns 10000 (100%). `range = 1400 - 600 + 1 = 801`. `seed % 801` gives uniform [0,800], mapped to [600,1400] BPS. | ETH-DIST | pending |
| D14 | `_hasActualTraitTickets()` | 2595-2609 | `traitBurnTicket[lvl][traitIds[i]].length` | View: checks for non-virtual (real) trait tickets. Unlike D1, does NOT check deity virtual entries. Used for carryover source selection. | CARRYOVER | pending |
| D15 | `_selectDailyCoinTargetLevel()` | 2328-2338 | `_hasTraitTickets()` (D1) | View: picks random level in [lvl, lvl+4] for coin jackpot. `entropy % 5` for uniform level selection. Returns 0 if chosen level has no eligible tickets. | COIN-JACKPOT | pending |
| D16 | `_calcAutoRebuy()` | PU:38-72 | Pure: `EntropyLib.entropyStep()`, `PriceLookupLib.priceForLevel()` | Pure (PayoutUtils): computes auto-rebuy parameters. **Confirmed pure:** Zero storage reads, zero storage writes. Level offset: `(entropy & 3) + 1` gives [1,4] levels ahead. `toFuture = (offset > 1)` means +1 -> next (25%), +2/+3/+4 -> future (75%). Take-profit: `(weiAmount / takeProfit) * takeProfit` rounds down. Bonus: 130% base, 145% afKing. | BAF-PAYOUT | pending |
| D17 | `_validateTicketBudget()` | 1024-1031 | `_hasTraitTickets()` (D1) | View: zeros budget if no trait tickets exist for given level/traits. Simple gate function. | TICKET-DIST | pending |
| D18 | `_packDailyTicketBudgets()` | 2676-2687 | Pure: bit packing | Pure: packs counterStep (8 bits), dailyTicketUnits (64 bits @ 8), carryoverTicketUnits (64 bits @ 72), carryoverSourceOffset (8 bits @ 136). | PACKING | pending |
| D19 | `_unpackDailyTicketBudgets()` | 2689-2705 | Pure: bit unpacking | Pure: inverse of D18. Uses uint64 casts for ticket units (truncates to 64 bits). | PACKING | pending |
| D20 | `_selectCarryoverSourceOffset()` | 2631-2674 | `_highestCarryoverSourceOffset()` (D21), `_hasActualTraitTickets()` (D14) | View: selects random eligible carryover source offset in [1..highest]. Wrapping probe starting from random point. Returns 0 if no eligible offsets. | CARRYOVER | pending |
| D21 | `_highestCarryoverSourceOffset()` | 2613-2626 | `_hasActualTraitTickets()` (D14) | View: scans [1..5] offsets in reverse, returns highest with actual tickets. Returns 0 if none eligible. | CARRYOVER | pending |
| D22 | `_rollRemainder()` | 2024-2031 | Pure: `EntropyLib.entropyStep()` | Pure: `(rollEntropy % TICKET_SCALE) < rem` where TICKET_SCALE=100. Fractional ticket roll. rem in [0,99]. Fair if entropyStep output is uniform. | TICKET-DIST | pending |
