...
**Write policy.** READ-only after Task 14 atomic commit per D-253-CF-02 / D-257 / D-262 carry-forward chain. KNOWN-ISSUES.md UNMODIFIED at HEAD per default zero-promotion path. AUDIT-06 `JackpotBurnieWin.lvl` semantic-shift is documented in §3c of THIS deliverable (the v34→v35 audit deliverable IS the proper venue for delta-event semantic-shift disclosures); KNOWN-ISSUES.md is reserved for warden pre-disclosure of ongoing-protocol-behavior items (it is NOT a release-notes / migration / changelog venue, and a v34→v35 delta-event note is not a finding-shaped pre-disclosure). Zero F-35-NN finding blocks per D-265-FIND-01 default path. Zero awaiting-approval test files (1 v35 contract commit + 6 v35 test commits USER-APPROVED batched per `feedback_batch_contract_approval.md` per Phase 263 / 264 close). Per `feedback_never_preapprove_contracts.md`, the orchestrator does NOT pre-approve any contract change; vacuous this phase since no contract changes are proposed by agent (zero `contracts/` writes + zero `test/` writes by agent — hard constraint #1).
...
- AUDIT-06: `JackpotBurnieWin.lvl semantic-shift documented in §3c prose of this deliverable (the v34→v35 audit deliverable is the proper venue for delta-event semantic-shift disclosures; KNOWN-ISSUES.md is NOT a release-notes / migration / changelog venue, so this is NOT routed there)`
...
- REG-03: `4 KI envelope re-verifications: EXC-01..03 NEGATIVE-scope; EXC-04 RE_VERIFIED with Phase 264 STAT-01 chi² cross-cite; KNOWN_ISSUES_UNMODIFIED`
...
ANY false ⇒ Non-Promotion Ledger entry with the failing predicate identified. Default outcome at this milestone: `KNOWN-ISSUES.md` UNMODIFIED — zero F-35-NN finding blocks emit (D-265-FIND-01 default path) → zero KI promotion candidates. AUDIT-06 indexer semantic-shift was considered for D-09 routing but does NOT qualify as a KNOWN-ISSUES.md candidate in the first place: KNOWN-ISSUES.md serves as warden pre-disclosure for ongoing-protocol-behavior items that a warden might mistake for a finding (e.g., Lido stETH dependency, deity-boon deterministic fallback, EntropyLib XOR-shift PRNG); a v34→v35 delta-event semantic-shift note is a release-notes / indexer-migration concern, not a warden pre-disclosure. AUDIT-06 lives in §3c of THIS deliverable (the v34→v35 audit deliverable is the proper venue for documenting the delta). Any v35-discovered finding-candidate (none emitted) would FAIL the **sticky** predicate (v35 per-pull-level surface is freshly-landed not "ongoing protocol behavior" until the next milestone). See §6 KI Gating Walk + Non-Promotion Ledger.
...
**D-09 gating disposition** (full row in §6b): D-09 was considered but determined NOT-APPLICABLE for this surface. The 3-predicate test (accepted-design + non-exploitable + sticky) all hold for the per-pull-level design, but D-09 routes finding-shaped surfaces that a warden might pre-empt by checking KNOWN-ISSUES.md. A v34→v35 delta-event semantic-shift note is not a finding-shaped surface — a warden has nothing to compare v34 against and would not submit "the lvl field changed semantics between versions" as a finding. The proper venue for the disclosure is THIS deliverable (`audit/FINDINGS-v35.0.md` §3c) which audits the v34→v35 delta; KNOWN-ISSUES.md is reserved for ongoing-protocol-behavior items that look finding-shaped at any single point in time. Closure verdict: `0 of 0 KI_ELIGIBLE_PROMOTED; KNOWN_ISSUES_UNMODIFIED`.
...
**Methodology note for AUDIT-06:** the `JackpotBurnieWin.lvl` semantic-shift surfaced in §3c was initially routed through D-09 gating but determined NOT-APPLICABLE (not a finding-shaped surface — see §3c "D-09 gating disposition" paragraph). KNOWN-ISSUES.md is reserved for warden pre-disclosure of ongoing-protocol-behavior items (e.g., Lido stETH dependency, deity-boon deterministic fallback, EntropyLib XOR-shift PRNG); a v34→v35 delta-event semantic-shift note is a release-notes / indexer-migration concern documented in THIS deliverable's §3c, not a warden pre-disclosure. KNOWN-ISSUES.md remains UNMODIFIED at HEAD.
...
| AUDIT-06 | `JackpotBurnieWin.lvl` semantic shift (call-level → per-pull-sampled-level) | `_awardDailyCoinToTraitWinners` helper at `contracts/modules/DegenerusGameJackpotModule.sol`; event L96 byte-identical | **D-09 NOT-APPLICABLE — disclosure venue is §3c of this deliverable, NOT KNOWN-ISSUES.md** | The 3-predicate test (accepted-design + non-exploitable + sticky) all hold for the per-pull-level design, but D-09 routes finding-shaped surfaces that a warden might pre-empt by checking KNOWN-ISSUES.md. A v34→v35 delta-event semantic-shift note is not a finding-shaped surface — a warden has nothing to compare v34 against and would not submit "the lvl field changed semantics between versions" as a finding. KNOWN-ISSUES.md is reserved for ongoing-protocol-behavior items that look finding-shaped at any single point in time (Lido stETH dependency, deity-boon fallback, EntropyLib XOR-shift, etc.); the v34→v35 delta is documented in §3c of this deliverable (the v34→v35 audit deliverable is the proper venue). KNOWN-ISSUES.md UNMODIFIED at HEAD. |
...
Detail: zero candidates emit from §4 (default zero F-35-NN blocks per D-265-FIND-01); zero promotions land. AUDIT-06 indexer semantic-shift was considered for D-09 routing but determined NOT-APPLICABLE (not a finding-shaped surface — see §3c "D-09 gating disposition" paragraph + §6b AUDIT-06 row). EXC-01..03 NEGATIVE-scope at v35 (no per-pull-level path interaction); EXC-04 RE_VERIFIED with Phase 264 STAT-01 chi² empirical cross-cite (range=4 chi²=5.114 < 7.815; range=8 chi²=3.019 < 14.067). Per `feedback_rng_backward_trace.md` + `feedback_rng_commitment_window.md`, the per-pull-level keccak path consumes VRF-derived bits and is post-commit unknown to player — uniformity claim covered end-to-end.
...
| §6b D-09 PASS rows | 0 | AUDIT-06 considered for D-09 routing but determined NOT-APPLICABLE (not a finding-shaped surface — see §3c + §6b row) |
| §6c Verdict Summary | 1 | "0 of 0 KI_ELIGIBLE_PROMOTED; KNOWN_ISSUES_UNMODIFIED" |
...
4. **Outcome.** 6 of 6 §4 adversarial surfaces SAFE / SAFE_BY_DESIGN / SAFE_BY_STRUCTURAL_CLOSURE + STAT-03 reframe row SAFE_BY_STRUCTURAL_CLOSURE (per D-265-STAT03-01); zero F-35-NN finding blocks; 2 PASS REG-01..02 + 9 PASS + 1 SUPERSEDED REG-04 rows; 4 KI envelope re-verifications (3 NEGATIVE-scope + 1 RE_VERIFIED with STAT-01 cross-cite); KNOWN-ISSUES.md UNMODIFIED at HEAD (AUDIT-06 indexer semantic-shift documented in §3c of this deliverable, not promoted to KNOWN-ISSUES.md — KNOWN-ISSUES.md is reserved for warden pre-disclosure of ongoing-protocol-behavior items, not v34→v35 delta-event notes).
