...
> This is a draft. Finalize before submission.
...
The protocol deploys 24 contracts: 14 core + 10 delegatecall modules sharing a single storage layout. Supporting infrastructure includes a dual-token system (sDGNRS soulbound + DGNRS transferable wrapper), BURNIE token with coinflip mechanics, stETH yield via DegenerusVault, deity pass NFTs with triangular pricing, and sDGNRS-holder governance for emergency VRF coordinator rotation. All contract addresses are compile-time constants via nonce prediction. Nothing is upgradeable.
...
See `KNOWN-ISSUES.md` in the repository root. Pre-audited with Slither v0.11.5 (1,959 raw findings, 32 detectors) and 4naly3er (4,453 instances, 81 categories). 27 DOCUMENT entries covering every Slither/4naly3er finding worth reporting, plus 5 ERC-20 deviations and 30 event audit findings.
...
| DegenerusAdmin | Admin operations, VRF governance (sDGNRS vote required) |
