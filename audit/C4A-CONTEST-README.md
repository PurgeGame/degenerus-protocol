...
**Frozen subject:** `contracts/` tree `49b36da3` @ tag `degenerus-c4a`.
...
| Group | Files | nSLOC |
|-------|------:|------:|
| Core deployed contracts | 14 | 7,460 |
| Deployed delegatecall game modules | 12 | 9,707 |
...
> The properties below are what the protocol guarantees; each is backed by the referenced test or source site. Numeraire: ETH wei (stETH valued 1:1 with ETH). Subject: the frozen `contracts/` tree `49b36da3` @ tag `degenerus-c4a`. `DEG-*` entries are statistical (realized EV/RTP/ROI over the resolved-spin stream); the rest are exact.
...
npm install
git checkout degenerus-c4a          # the frozen subject (contracts/ tree 49b36da3)
