...
**Frozen subject:** `contracts/` tree `9777a3f7` @ tag `degenerus-c4a`.
...
| Deployed delegatecall game modules | 12 | 9,718 |
...
> The properties below are what the protocol guarantees; each is backed by the referenced test or source site. Numeraire: ETH wei (stETH valued 1:1 with ETH). Subject: the frozen `contracts/` tree `9777a3f7` @ tag `degenerus-c4a`. `DEG-*` entries are statistical (realized EV/RTP/ROI over the resolved-spin stream); the rest are exact.
...
npm install
git checkout degenerus-c4a          # the frozen subject (contracts/ tree 9777a3f7)
