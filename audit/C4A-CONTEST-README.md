...
**Frozen subject:** `contracts/` tree `f79fda85` @ tag `degenerus-c4a`.
...
> The properties below are what the protocol guarantees; each is backed by the referenced test or source site. Numeraire: ETH wei (stETH valued 1:1 with ETH). Subject: the frozen `contracts/` tree `f79fda85` @ tag `degenerus-c4a`. `DEG-*` entries are statistical (realized EV/RTP/ROI over the resolved-spin stream); the rest are exact.
...
npm install
git checkout degenerus-c4a          # the frozen subject (contracts/ tree f79fda85)
