...
> Canonical runtime oracle shared verbatim by the adversarial agent (`agent/src/oracle.js`, which asserts a subset at runtime) and this section of the C4A README. Subject (re-pinned Phase 474): v74 frozen `contracts/` tree `280bdb19` @ impl `3986926c`; closure `MILESTONE_V74_AT_HEAD_PENDING_478` (the concrete terminal sha is emitted Phase 478). Verdict: PROVISIONAL — 0 findings from the 8-cluster as-built audit (Phases 468-473); the cross-model adversarial re-audit (Phase 475) is still pending. Numeraire: ETH wei (18 decimals); stETH valued 1:1 with ETH (protocol sums at parity).
