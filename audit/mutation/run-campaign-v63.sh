#!/usr/bin/env bash
# Corrected mutation campaign for the v63 frozen subject a8b702a7.
#
# Fix-site/spine-scoped (TARGETS-v63.md), driven against the COMPREHENSIVE oracle
...
#
# Usage:
#   run-campaign-v63.sh                       # full campaign, fix-site order, resumable
#   run-campaign-v63.sh --single <ContractName>  # run/resume ONE target (paced background)
set -uo pipefail
...
mkdir -p "$OUT"
PROG="$OUT/PROGRESS-v63.log"
ORACLE="$OUT/oracle-comprehensive.sh"

# ----- Subject pin (the byte-freeze the campaign must never violate) -----
SUBJECT_SHA="a8b702a7"
SUBJECT_TREE="2934d3d8987a09c5f073549a0cb499f6c5f28620"
SRC="contracts"
...
echo "campaign-v63 START $(date -u +%FT%TZ) HEAD=$(git rev-parse --short HEAD) subject=$SUBJECT_SHA" >> "$PROG"
...
run_target() {
  ...
  slither-mutate "$REPO/$relpath" \
    ...
    --timeout 600 \
    --output-dir "$OUT/$name-mut-v63" \
    -v > "$OUT/$name-v63.log" 2>&1 || true
  restore
  assert_frozen "after $name"
  t1=$(date +%s)
  unc=$(grep -ciE 'UNCAUGHT' "$OUT/$name-v63.log" 2>/dev/null || true)
  kil=$(grep -ciE 'CAUGHT' "$OUT/$name-v63.log" 2>/dev/null || true)
...
if [ "${1:-}" = "--single" ]; then
  ...
  for entry in "${TARGETS[@]}"; do
    ...
    if [ "$name" = "$want" ]; then
      ...
      echo "campaign-v63 SINGLE-END $name $(date -u +%FT%TZ)" >> "$PROG"
      exit 0
    fi
  done
...
echo "campaign-v63 END $(date -u +%FT%TZ)" >> "$PROG"
