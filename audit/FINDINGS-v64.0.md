...
### 401 — PACKING & GAS-IDENTITY (full council gemini+codex + NET-2 + deterministic scripts)
...
- **Codex backfill — 4 narrowing/RMW flags, all REFUTED as unreachable** (the cross-model net's type-max lens; PACK-03/04 codex also HOLDS): `_setFlipStake` over-wide stake, `autoRebuyStop`/`autoRebuyCarry`/`claimableStored` narrowings, `Sub.amount` milli-ETH stamp — each bounded below its width by the **documented uint128 BURNIE supply cap** (`_toUint128` reverts `SupplyOverflow`; `_setFlipStake:1183-1185` documents the invariant in-code) + realistic scale (BURNIE supply ~14 orders of magnitude under the cap; the overflow mint reverts; 16,777-ETH single auto-buy unreachable and the ETH debit is full-wei-correct). INFO/defense-in-depth.
...
### 402 — PERMISSIONLESS-COMPOSITION (full council gemini+codex + NET-2)
- **PERM-01..04 — attested.** Permissionless claims credit the named beneficiary never `msg.sender`; CEI holds; decimator offset-key `lvl`/`lvl+1` isolation holds (DEC-ALIAS class); keeper bounties net-negative vs real gas; indexer events emission-only. **Codex backfill = ALL FOUR HOLD, no findings** — independently corroborates the by-design dispositions (it confirms `MintStreakRecorded` correctly scopes to the manual-mint quest path and raised no PERM-CRIT-01 escalation).
...
- v64 subject: **BurnieCoinflip** launched on the re-pinned `run-campaign-v64.sh` — the comprehensive oracle **BASELINE_CHECK passed green** and early mutants scored (constructor `:204`/`:205` `==> revert()` → **CAUGHT**), then **stopped clean** (restore trap + `git checkout` → subject re-frozen `402855e1`, verified) because the per-mutant via_ir scoring is overnight-scale and transiently dirties the working tree (blocking commits). **CI-resumable.** Lootbox + Decimator likewise CI-resumable. The v64-changed functions in all three are primarily covered by the 399–403 dual-net sweeps (all clean); 0 contract defects expected. Matches the v63 bounded mutation close. *(See `404-MUTATION-STATUS.md`.)*
...
Every no-finding verdict has both nets on record (council + Claude Workflow). Codex hit a usage cap mid-run (phases 401/402 ran gemini + NET-2 + deterministic scripts/orchestrator cross-check **in-phase**; codex was **backfilled after the cap reset** — 401: 4 narrowing flags all refuted unreachable; 402: clean, corroborating; 403 onward had the full council in-phase). The cross-model council earned its keep twice: **RWD-A** (the seed-emission EV arithmetic the other nets glossed → USER-confirmed by-design) and the **401 codex narrowing flags** (the type-max concerns a warden would raise, documented as bounded). No Write-capable subagent mutated the frozen contract source (git-verified after every fan-out; the mutation campaign's transient edits were restore-trapped + re-frozen, verified clean).
