...
Frozen subject: `contracts/` tree `58367471` @ tag `degenerus-c4a` (post-v76.0 audit freeze).
...
- **Genesis feed is a compile-time constant, not a governance grant.** `ContractAddresses.LINK_ETH_FEED`
  is patched at deploy and installed by `DegenerusAdmin`'s constructor, so donations are valued from
  block one; the deployer picks it the way it picks every other address in that file, and the
  constructor performs no oracle call and no decimals check. `address(0)` — the checked-in default —
  leaves the slot empty, which reads as maximally stalled and hands the choice straight to the swap
  path above. A wrong or hostile feed pinned here is bounded exactly as a voted-in one is: the
  `LINK_ETH_MAX_PRICE = 0.05 ether` valuation ceiling caps the FLIP credit it can mint, a stale or
  reverting answer values at zero rather than blocking donations, and governance swaps it out once
  it goes unhealthy.
...
**Powers:** none over the protocol. Eighteen player-facing contracts — the sixteen standalone
...
raw-selector gate carries an explicit justification comment at each of the 17 call sites.
