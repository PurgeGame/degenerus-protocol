...
///         `(uint32(entropy >> 96) % uint32(TICKET_SCALE)) < frac`; if either drifts,
...
contract JackpotBernoulliTester {
        ...
        returns (uint32 whole, bool roundedUp)
    {
        ...
        uint32 frac = scaledTickets % uint32(TICKET_SCALE);
        roundedUp = false;
        if (frac != 0 && (uint32(seed >> 96) % uint32(TICKET_SCALE)) < frac) {
            unchecked {
                whole += 1;
            }
            roundedUp = true;
