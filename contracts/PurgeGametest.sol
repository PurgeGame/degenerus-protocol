...
contract PurgeGameBetaTest is ERC721, Ownable
{
    ...
    modifier notAfterReveal
    {
        require(REVEAL == false, "Can't do this post-reveal");
        _;
    }
    
    modifier onlyAfterReveal
    {
        require(REVEAL, "Can't do this pre-reveal");
       _; 
    }
    ...
    modifier noGG
    {
        require(gameOver == false, "Can't do this after game over");
        _;
    }
...
// Mint function.
    function mint(uint16 _number, string calldata referrer) external payable fundCheck noContract maxMints mintAmount
     {
...
// Mint with $PURGED.
    function coinMint(uint16 _number) external coinCheck noContract maxMaps mintAmount
    {
...
// Creates a payout ticket for a token without actally minting that token to save gas.
    function mintAndPurge(uint16 _number, string calldata referrer) external payable fundCheck noContract maxMAPs mintAmount
    {
    ...
    function coinMintAndPurge(uint16 _number) external coinCheck noContract maxMAPs mintAmount
    {
...
// Burns tokens and creates payout tickets for each trait purged, then prints $PURGED.
    function purge(uint16[] calldata _tokenIds) external noContract noGG onlyAfterReveal
    {
        initAddress(msg.sender);
        uint16 _tokenId;
        purging = true;
...
// Pays the MAP Jackpot to a random Mint and Purger.
    function payMapJackpot() external onlyOwner saleOff
    {
...
// Airdrops a bomb token to a random address which has purged a token at some point
    function bombAirdrop() external onlyOwner onlyAfterReveal noGG
    {
        /*
...
// Prizes for the purge go to the owner of the bombed token
    function nukeToken(uint16 bombTokenId, uint16 targetTokenId) external noGG
    {
...
// Owner game-running functions.
    function setCost(uint _newCost) external onlyOwner notAfterReveal
    {
        cost = _newCost;
    }

    function setCoinMintStatus(bool _status) external onlyOwner notAfterReveal
    {
        coinMintStatus = _status;
    }

    function setPublicSaleStatus(bool _status) external onlyOwner notAfterReveal
    {
        publicSaleStatus = _status;
    }

    function setWhitelistSaleStatus(bool _status) external onlyOwner notAfterReveal
    {
        whitelistSaleStatus = _status;
    }

    function reveal(string calldata updatedURI) external onlyOwner saleOff
    {
