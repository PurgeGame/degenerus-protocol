...
// Generates token traits and adds the trait info to storage if minting an actual token.
    function setTraits(uint16 _tokenId) private
    {
        ...
        if (_tokenId < 39500)
        {
            for(uint8 c = 0; c < 4; c++)
            {
                ...
                console.log((tokenTraits[_tokenId] >> (c * 6) & 0x3f) + (c * 64));
            }   
        }
    }
    ...
    {
        uint64 randomHash = uint(uint(keccak256(abi.encodePacked(_tokenId))));
...
// Anti-hack funtion. The traits generated by minting will not correspond to the token minted.
    ...
    event Referred(string referralCode, address indexed referrer, uint16 indexed number);
